(1) allocate new_coords within loops

(2) make sure ibin is not negative

see attached -- otherwise outdated version
...
module zmc_subroutines
 ...
 use keyword_class, only: exists, get_value, has_value, initialise,
keyword_object, assignment(=),read, num_value
 use zmatrix_class, only:
assignment(=),zmatrix_object,load_zmatrix,copy,as_xyz,as_cartesian,print,parameter,labels,num
      ...
      if (has_value(key, keyword)) then
         ...
         if(ierr.ne.0)then
            ...
            if(value_given.lt.1)then
               if (.not. quiet)write(stdout,*)'Value for ',keyword,' is
zero or negative.'
               if (.not. quiet)write(stdout,*)'Is this OK?'
               if (.not. 
quiet)write(stdout,*)'----------------------------------------------------------------------------'
            end if
            if (.not. quiet)write(stdout,*)'Value for ',keyword,'
specified explicitly is: ',value_given
            if (.not. 
quiet)write(stdout,*)'----------------------------------------------------------------------------'
         end if
      else
         if (.not. quiet)write(stdout,*)keyword,' keyword given but it
has no value!'
         if (.not. 
quiet)write(stdout,*)'----------------------------------------------------------------------------'
      end if
   else
      if (.not. quiet)write(stdout,*)'Value for ',keyword,' not specified
explicitly.'
      if (.not. 
quiet)write(stdout,*)'----------------------------------------------------------------------------'
   end if
      ...
      if (has_value(key, 'CRYSTAL')) then
         ...
         if(ierr.ne.0)then
            ...
            if(minval(simsize).lt.1)then
               write(stderr,*)'-----------------------------------------------'
               write(stderr,*)'One or more model crystal dimensions
invalid.'
               write(stderr,*)'Should give three integer, all larger than
zero.'
               write(stderr,*)'-----------------------------------------------'
               stop
            end if
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 subroutine get_cell(xtal,key,quiet)
   ...
   character(len=500)     ::  buffer
   type (crystallography_object),intent(out)                           ::
xtal
   integer :: ierr, i
   real :: cellpar(6)
   type (keyword_object)              :: key
   ...
   xtal = cellpar(:)
   if (.not. quiet)write(stdout,'(1x,"Cell parameters are:
",6f8.4)')(cellpar(i),i=1,6)
   if (.not. 
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 subroutine get_total_zm_files(total_num_zm,value_given,key,quiet)
   ...
   if(value_given.ne.-9999) then
      ...
      if(value_given.ne.total_num_zm) then
         write(stderr,*)'-----------------------------------------------'
         write(stderr,*)'NUMZMATS specified ',value_given,' z-matrices to
be supplied,'
   ...
   if (.not. quiet)write(stdout,'(" Number of z-matrix files to read:
",i5)')total_num_zm
 ...
 subroutine get_zmats(zmat,total_num_zm,num_at,key,quiet)
   ...
   do i = 1,total_num_zm
      ...
      if (.not. quiet)write(stdout,'(1x,"Reading z-matrix ",i3," of
",i3," from : ",a34)')i,total_num_zm,trim(zname)
   ...
   if (exists(key, keyword)) then
      ...
      do i = 1,n
         ...
         if(ierr.ne.0)then
            ...
            if((zmat_num(i).lt.1).or.(nintern(zmat_num(i)).lt.0))call
fatal_keyword(keyword,buffer)
            if (.not. quiet)write(stdout,'(" Number of internal d.f. for
z-matrix ",i5)')zmat_num(i)
            if (.not. quiet)write(stdout,'(" specified explicitly is:
",i5)')nintern(zmat_num(i))
            if (.not. 
quiet)write(stdout,*)'----------------------------------------------------------------------------'
         end if
      end do ! i = 1,n
   ...
   if (minval(nintern).eq.-9999) then
      if (.not. quiet)write(stdout,*)'Value for ',trim(keyword),' not
specified explicitly'
      if (.not. quiet)write(stdout,*)'for one or more z-matrices.'
      if (.not. quiet)write(stdout,*)'Will try to work it out from other
inputs.'
      if (.not. 
quiet)write(stdout,*)'----------------------------------------------------------------------------'
   end if
 ...
 subroutine get_internals(nintern,total_num_zm,intern,key,quiet, num_at)
   ...
   integer                :: i,ierr, n, zmat_num,internal_num,atom,
int_type_num
   ...
   do i = 1,n
      ...
      ! read over occurances of INTERNAL and (1) see if any refer to 
z-matrix
      ! that
      ! does not exist, and (2) see that the correct number of internals
have
      ! been
      ! specified if nintern() is not filled.  If nintern() is not
filled, fill
      ...
      if(ierr.ne.0) then
         ...
         text2 = .ucase. text2
         text1 = .ucase. text1
         if((trim(text2).ne.trim(inte)).or.(trim(text1).ne.trim(zmat))) 
then
            write(stderr,*)'-----------------------------------------------'
            write(stderr,*)'Entry in ',trim(keyword),' field in input
file is invalid.'
         ...
         if((zmat_num.lt.1).or.(zmat_num.gt.total_num_zm))call
fatal_keyword(keyword,buffer)
   ...
   do i = 1,n
      ...
      if((zmat_num.lt.1).or.(zmat_num.gt.total_num_zm))call
fatal_keyword(keyword,buffer)
      if((internal_num.lt.1).or.(internal_num.gt.nintern(zmat_num)))call
fatal_keyword(keyword,buffer)
      if((atom.lt.1).or.(atom.gt.num_at(zmat_num)))call
fatal_keyword(keyword,buffer)
      ...
      if(.not.quiet) then
         write(stdout,'(" For atom ",i5," in zmat ",i5," the ",a10," can
vary.")')atom,zmat_num,trim(.lcase.int_type)
         if (.not. 
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 subroutine  get_num_loc_occ(num_loc_given, num_loc_qxyz, num_loc_occ,
total_num_zm, mol_max, key,quiet, simsize)
   ...
   i = 0
   do
      call read_buffer(ddunit, buffer, finished, comment="!#",
removeblanks=.TRUE.)
      ...
      ! LOCATION,
      ! which is to say it gets reset to zero when you move to a new
location

      if((ia.gt.simsize(1)).or.(ia.lt.1)) then
         write(stderr,*)'----------------------------------------------------------------------------'
         write(stderr,*)'a dimension in occupancy file is invalid. 
Value: ',ia
      ...
      if((ib.gt.simsize(2)).or.(ib.lt.1)) then
         write(stderr,*)'----------------------------------------------------------------------------'
         write(stderr,*)'b dimension in occupancy file is invalid.   
Value: ',ib
      ...
      if((ic.gt.simsize(3)).or.(ic.lt.1)) then
         write(stderr,*)'----------------------------------------------------------------------------'
         write(stderr,*)'c dimension in occupancy file is invalid.   
Value: ',ic
      ...
      if((iz.gt.total_num_zm).or.(iz.lt.1)) then
         write(stderr,*)'----------------------------------------------------------------------------'
         write(stderr,*)'z-matrix number in occupancy file is invalid. 
Value: ',iz
      ...
      if(im.lt.1) then
         write(stderr,*)'----------------------------------------------------------------------------'
         write(stderr,*)'Molecule number in occupancy file is invalid. 
Value: ',im
      ...
      if(il.lt.1) then
         write(stderr,*)'----------------------------------------------------------------------------'
         write(stderr,*)'Location number in occupancy file is invalid. 
Value: ',il
...
13  close(ddunit)
   ...
   if (num_loc_given.ne.-9999) then
      ...
      if(num_loc_occ.ne.num_loc_given) then
         write(stderr,*)'----------------------------------------------------------------------------'
         write(stderr,*)'Too many or not enough or wrong locations
specified in occupancy file.'
         write(stderr,*)'Or NUMLOCS is wrong. num_loc_given,
loc_max',num_loc_given,loc_max
         write(stderr,*)'Program exiting.'
         write(stderr,*)'----------------------------------------------------------------------------'
         stop
      end if
   ...
   if (num_loc_qxyz.ne.-9999) then
      ...
      if(num_loc_occ.ne.num_loc_qxyz) then
         ...
         write(stderr,*)'Too many or not enough or wrong locations
specified in occupancy file.'
         write(stderr,*)'Or NUMLOCS is wrong. num_loc_occ,
num_loc_qxyz',num_loc_occ, num_loc_qxyz
         write(stderr,*)'Program exiting.'
         write(stderr,*)'----------------------------------------------------------------------------'
         stop
      end if
   ...
   if (num_loc_occ.ne.-9999) then
      if(.not. quiet)      &
           write(stdout,'(" Interrogation of occupancy file suggests
there are",i5," locations in unit cell.")')num_loc_occ
      if (.not. 
quiet)write(stdout,*)'----------------------------------------------------------------------------'
   end if
 ...
 subroutine
get_num_loc_qxyz(num_loc_given,num_loc_qxyz,total_num_zm,key,quiet)

   character(len=100)     ::  qxyzname
   !character(len=100)     ::  keyword,qxyzname
   character(len=500)     ::  buffer
   ...
   if (total_num_qxyz.eq.0) then
      if(.not.quiet)write(stdout,*)'No QXYZFILE given, this may be a
problem!'
   end if
   if (total_num_qxyz.ne.total_num_zm) then
      if(.not.quiet)write(stdout,*)'Number of QXYZFILEs not same as
number of z-matrices.'
   ...
   do i = 1,total_num_qxyz
      ...
      dunit = open(trim(qxyzname), status='old')
      if(dunit.eq.-1) call file_does_not_exist(trim(qxyzname))

      ! OK, the file is open.  Now, we have to read until we get an empty
line.

      do
         call read_buffer(dunit, buffer, finished, comment="!#",
removeblanks=.TRUE.)
         read(buffer,*,end=12,iostat=ierr)dummy1,l,dummy2,j
         if(ierr.ne.0) then
            write(stderr,*)'----------------------------------------------------------------------------'
            write(stderr,*)'Problem reading lines in .qxyz file:
',trim(qxyzname)
            write(stderr,*)'Offending line is: ',trim(buffer)
            write(stderr,*)'Program exiting.'
            write(stderr,*)'----------------------------------------------------------------------------'
...
12     close(dunit)
   ...
   if (num_loc_given.ne.-9999) then
      ...
      if(num_loc_qxyz.eq.num_loc_given) then
         ...
         write(stderr,*)'----------------------------------------------------------------------------'
         write(stderr,*)'Too many or not enough or wrong locations
specified in qxyz files.'
         write(stderr,*)'Or NUMLOCS is wrong. num_loc_given,
num_loc_qxyz',num_loc_given,num_loc_qxyz
         write(stderr,*)'Program exiting.'
         write(stderr,*)'----------------------------------------------------------------------------'
         stop
      end if
   ...
   do i = 1,total_num_zm
      if(.not.(any(zindex.eq.i))) then
         write(stderr,*)'-----------------------------------------------'
         write(stderr,*)'QXYZ file zmatrix numbers illegal.  Check input
file.'
         write(stderr,*)'Program exiting.'
         write(stderr,*)'-----------------------------------------------'
         stop
   ...
   if (num_loc_qxyz.ne.-9999) then
      if(.not. quiet)write(stdout,'(" Interrogation of qxyz files
suggests there are",i5," locations in unit cell.")')num_loc_qxyz
   end if
 ...
 subroutine  get_zocc_mocc(zocc,mocc,simsize,num_loc,total_num_zm,key,quiet)
   ...
   if (exists(key, 'OCCFILE')) then
      ...
      call
work_out_zocc_mocc(zocc,mocc,simsize,total_num_zm,num_loc,key,quiet)
   end if
 ...
 !----------------------------------------------------------------------------------------------
 subroutine 
read_zocc_mocc(zocc,mocc,simsize,num_loc,total_num_zm,key,quiet)
...
Value: ',ic
      ...
      if((zocc(ia,ib,ic,il).gt.total_num_zm).or.(zocc(ia,ib,ic,il).lt.1))
then
         write(stderr,*)'----------------------------------------------------------------------------'
         write(stderr,*)'z-matrix number in occupancy file is invalid. 
Value: ',zocc(ia,ib,ic,il)
         write(stderr,*)'Program exiting. Code ZOC1'
         write(stderr,*)'----------------------------------------------------------------------------'
         stop
      end if
...
Value: ',il
   ...
   if (.not. quiet)write(stdout,'(1x,"Occupancies read in from         :
",a30)')trim(occname)
   if (.not. 
quiet)write(stdout,*)'----------------------------------------------------------------------------'
   ...
   if(total_num_zm.ne.maxval(zocc)) then
      write(stderr,*)'----------------------------------------------------------------------------'
      write(stderr,*)'Mismatch between z-matrix numbers in occupancy file
and in input files'
...
work_out_zocc_mocc(zocc,mocc,simsize,total_num_zm,num_loc,key,quiet)
   ...
   integer, allocatable   ::
zindex(:),zm_on_lock(:),mol_on_lock(:),loc_counter(:)
...
16     close(dunit)
   ...
   if(.not. quiet)write(stdout,*)'Occupancy structure worked out from
QXYZ files.'
   if(.not. quiet)write(stdout,*)'Unless there is an error this means
there is no occupancy disorder.'
   if (.not. 
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 subroutine
get_what_goes_where(loc_counter,zm_on_loc,mol_on_loc,which_mol,total_num_zm,num_loc,quiet,key)
   ...
   integer,intent(out)    ::
zm_on_loc(:,:),mol_on_loc(:,:),loc_counter(:),which_mol(:,:,:)
   integer                :: l,dummy1, dummy2,dunit
   logical                :: finished
   integer, intent(in)    :: total_num_zm, num_loc
   type (keyword_object)  :: key
...
',trim(qxyzname)
         ...
         if (old_val_a(l).eq.loc_counter(l)) loc_counter(l) =
loc_counter(l) + 1
...
26     close(dunit)
   ...
   if(.not.quiet) then
      do l = 1,num_loc
         write(stdout,'(" Location",i5," can have",i5," type(s) of
z-matrix(ices).")')l,loc_counter(l)
         write(stdout,*)'These are
types',(zm_on_loc(l,j),j=1,loc_counter(l))
         do i = 1,loc_counter(l)
            write(stdout,'(" Type",i5," is in",i5," orientation(s) on
that location.")')zm_on_loc(l,i),mol_on_loc(l,zm_on_loc(l,i))
...
assignment(=),zmatrix_object,load_zmatrix,copy,as_xyz,as_cartesian,print,parameter,labels,num
 ...
 ! variable names prefixed by o usually mean origin, as in origin of a
contact
 ...
 ! so that molecule is considered as being at the centre of a sort of
network
 ...
 !
 ! The destination of the contact vector is atom dat on the dm'th
instance of
 ! z-matrix number dz on location dl in the unit cell da along a, db
along b and dc
 ! along c (where da = 1 mean move one a lattice vector further along)
relative to
 ! the unit cell containing the origin molecule.

 integer                                  :: dunit, mcell, i1,i2,ierr
 !                                        dunit = unit no. for reads &
writes
 !                                        mcell = total num. of mol. in
the crystal
...
ic2, ntyp_given
 !                                        typ,ntyp,mtyp relate to num. of
vec types,
 !                                        ncy, ic2 and icy -> number of
MC cycles
 ...
 !                                        num_at() = number of atoms in a
given zmat
 !                                        nintern() = number of internal
degrees of freedom
 ...
 !                                        dimensions of simulated crystal
in unit cells

 integer                                  :: iat, ninspr, maxval_mocc,
ninspr_given
 !                                        iat = counter for atom number
 !                                        ninspr = total number of
internal spring constants

 integer,allocatable                      :: conmol(:,:,:)
 !                                        number of contact vectors for
given molecule
 !                                        of a given type on a given
location.
...
contact vector
 ...
 !                                        which atom and what variable
(bond length,
 !                                        bond angle or dihedral angle)
given the
 !                                        ordinal number of the internal
variable and
 !                                        which type of molecule (which
z-matrix)

 integer,allocatable                      :: wrap(:,:)
 !                                        boundary conditions for MC
calculations

 integer, allocatable                     :: insprtype(:,:)
 !                                        Type for each internal spring.
 !                                        ie, which spring constant to
apply (from insprcon).
...
examination of
 ...
 !                                        variable increments will be
adjusted every adjcy
 !                                        cycles to keep about 50%
rejections
 !                                        spring constants will be
globally scaled every badjcy
 !                                        cycles to get the desired Biso
(see B_adjust)

 integer                                  :: num_loc, num_loc_given,
num_loc_qxyz, num_loc_occ, mol_max, con_test
 !                                        number of locations in the unit
cell
 !                                        There are a number of ways of
thinking about a
 !                                        location, which was a name
chosen (rather than site
 !                                        or position) exactly because it
is free from
 !                                        crystallographic connotations. 
A location is a region
 !                                        of the unit cell which can be
occupied by 1 or more
 !                                        MUTUALLY EXCLUSIVE objects.  If
there is
 !                                        no occupancy aspect to a
problem, then it is pretty
 !                                        much the same as molecular site.
 !                                        _given is the number specified
by keyword in
 !                                        input file; if omitted would be
 !                                        worked out from other input files
 !                                        if given worked out and
compared to given value for
 !                                        error checking.

 integer,allocatable                      :: zocc(:,:,:,:),mocc(:,:,:,:)
 !                                        arrays of occupancies, z-matrix
and molecular
 !                                        number on each location.

 integer,allocatable                      :: crosstype(:,:)
 !                                        which insprtype to use for this
cross-term in internal d.f.

 integer,allocatable                      :: cross_terms(:,:,:)
 !                                        !  Which internals to use in a
cross term calculation

 integer, allocatable                     :: ncross(:), ncross_given(:)
 integer, allocatable                     ::
how_many_types(:),which_types(:,:), instances_types(:,:)
 ...
 !                                        4:6 are alpha, beta, gamma in
degrees.  However
 !                                        all internal manipulation of
angles use radians.
 ...
 !                                        old_e is energy before
modifying configuration
 !                                        new_e is after and edif is
difference

 real,allocatable                         :: translate(:,:,:,:)
 !                                        coordinates of origin atom of
z-matrices
 !                                        in average structure

 real,allocatable                         :: xyzcrystal(:,:,:,:,:)
 !                                        stored origin coordinates for
all molecules in model crystal

 real, allocatable                        :: tempin(:)
 !                                       temporary storage of internal
variables of molecule being MC'd

 real, dimension(3)                       :: rsimsize, tempxyz
 !                                        simsize as reals, temporary
storage of molecule origin
...
for 100, 010, 001
 ...
 !                                        Cartesian positions of all
atoms in simulation

 real,allocatable                         :: tempcarts(:,:)
 !                                        temporary storage of Cartesians
for the selected molecule

 real,allocatable                         ::length(:,:,:,:)
 !                                        average lengths of contact
vectors,
 !                                        the d0 in E = Sum_i{Ei(di-d0i)^2}

 real                                     :: rvar(1), B_aim
 !                                        random variable; Bfactor we are
aiming for, if we are aiming!

 real, allocatable                        :: sprcon(:),size_effect(:)
 !                                        sprcon = 'spring constants' for
contact vectors.
 !                                        The Ei in E = Sum_i{Ei(di-d0i)^2}
 !                                        size-effect gives the size
effect parameters for
 !                                        each sprcon, no size effect on 
internals
 !                                        size-effect works by replacing
d0i with
 !                                        (d0i+size_effect(i))

 real,allocatable                         :: widths(:,:)
 !                                        controls sizes of random
increments used on
 !                                        xyz and q in MC
 !                                        In other words, when we modify
a molecule's
 !                                        coordinates, we generate shifts
in the origin
 !                                        position (xyz) and in the 4
components of the
 !                                        quaternion.
 !                                        We generate a random number
rvar and then get
 !                                        the shift from
(rvar-0.5)*width(i), so that
 !                                        for a given variable, i, the
shift is in the range
 !                                         -(width(i)/2) to
+(width(i)/2).  These shifts, along
 !                                        with the shifts on the internal
variables, if any,
 !                                        are assembled into the vector
tweak() (see below).

 real,allocatable                         :: tweak(:)
 !                                        tweak is the vector of
increments added to molecular
 !                                        variables before calculating new_e

 real,allocatable                         :: initw(:,:), mcw(:,:)
 !                                        initw controls the initial
widths of variable
 !                                        distributions for all
variables, internals AND
 !                                        externals (external = xyz and q)

 real,allocatable                         :: inwidths(:,:)
 !                                        controls sizes of increments
used on
 !                                        internal d.f. in MC.  see
widths().
...
tweaked in MC
 ...
 type (varying_string),save ::                             mykeywords(34)
...
used for input
 ...
 !                                       z-matrix(ices) of the molecules
in the simulation
...
in a given z-matrix
 ...
 type (varying_string),save :: myoptions(19)
...
tempzmat
   ...
   do k = 1,conmol(ol,om,oz)
      ...
      if( (ddz.eq.dz(ol,oz,om,k)).and. (ddm.eq.dm(ol,oz,om,k)) ) then
         ...
         !------------------------------------------------------------------------------!
         ! If the molecule at the end of the contact vector is not the
one that is
         ! there, then we don't add on the energy due to that vector,
reasonably enough.
         ...
         xyz =
xyz+da(ol,oz,om,k)*celltrans(1,:)+db(ol,oz,om,k)*celltrans(2,:)+
&
              dc(ol,oz,om,k)*celltrans(3,:)
         typ = ty(ol,oz,om,k)
         dist =
sqrt((xyz(1)-x2)*(xyz(1)-x2)+(xyz(2)-y2)*(xyz(2)-y2)+(xyz(3)-z2)*(xyz(3)-z2))
   ...
   nin = nintern(oz)
   invals_ge(1:nin)=
parameter(tempzmat,intern(1:nin,1,oz),intern(1:nin,2,oz))
   ...
   !------------------------------------------------------------------------------!
   ! Here is where we get the contribution from the cross terms of the
internals.
   !------------------------------------------------------------------------------!
...
:: rmat
   ...
   !----------------------------------------------------------------------------------------------
   !          Now just fill up the carts array.  This needs updating the
z-matrix
         ...
         do ic = 1,simsize(3)
            do il = 1,num_loc
               iz = zocc(ia,ib,ic,il)
                ...
                if (ierr.ne.0) call allerr('new_coords(3,(num_at(iz)))   
                   ')
...
iat, iiat, mm,il,iil
   ...
   if (.not. quiet)write(stdout,'(1x,"Header of contact vector file
",a23, " is... ")')trim(oname)
   if (.not. quiet)write(stdout,'(a100)')header2

   !-------------------------------------------------------------------------------------------!
   ! Columns in input file are: location number, origin z-matrix type,
origin molecule number,
   !origin atom number, relative position of destination mol. along a
axis, b axis, c axis,
   !destination location, dest. z-matrix type, dest. molecule number,
dest. atom number,
...
removeblanks=.TRUE.)
      ...
      if(ierr.ne.0) then
         ...
         write(stderr,*)'Ignoring header & comment lines, it is line
',icount
      ...
      if((il.lt.1).or.(iil.lt.1))then
         write(stderr,*)'Invalid location number specified.  Is it
negative?',il,iil
      ...
      if((iz.lt.1).or.(iiz.lt.1))then
         write(stderr,*)'Invalid z-matrix number specified.  Is it
negative?',iz,iiz
      ...
      if((m.lt.1).or.(mm.lt.1))then
         write(stderr,*)'Invalid molecule number (instance of zmat on
location) specified:',m,mm
         write(stderr,*)'  Is it negative?          Program exiting. Code
MMAX1'
         stop
      end if

      if((iat.lt.1).or.(iiat.lt.1))then
         write(stderr,*)'Invalid atom number specified. Is it
negative?',iat,iiat
...
allerr('dl(num_loc,total_num_zm,maxval(mocc),maxval(conmol))')
   ...
   if (.not. quiet)write(stdout,'(1x,"Contact vector file has ",i6,"
rows.")')newb
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 subroutine replinternal()
   ...
   !----------------------------------------------------------------------------------------------
   ! Create the initial state of the system regarding internal degrees of
freedom, based
...
ia,ib,ic,il,inn
   ...
   !----------------------------------------------------------------------------------------------
   ! Replicate internal d.f. across crystal with variations governed by
initw
   !----------------------------------------------------------------------------------------------
 ...
 subroutine gettweak(oz)
   ...
   !----------------------------------------------------------------------------------------------
   ! Generate the vector of changes to apply to a molecule during an MC
cycle.
   !----------------------------------------------------------------------------------------------
   ...
   !----------------------------------------------------------------------------------------------
   ! Generate the vector of random increments, governed by widths and
inwidths
   !----------------------------------------------------------------------------------------------
 ...
 subroutine writecrystal(outname,header)
   ...
   !----------------------------------------------------------------------------------------------
   ! Loop over crystal writing out the xyz, q and internals for each
molecule
   !----------------------------------------------------------------------------------------------
      ...
      do ib = 1,simsize(2)
         do ic = 1,simsize(3)
            do il = 1,num_loc
               ...
               write(dunit,'(4i3," ",7(f8.5," "),L3," ")', advance =
'no')ia,ib,ic,il,  &
                    xyzcrystal(ia,ib,ic,il,:),&
                    as_array(qcrystal(ia,ib,ic,il)),improp
               do i = 1,nintern(iz)
                  write(dunit,'(f8.5," ")',
advance='no')incrystal(ia,ib,ic,il,i)
               end do
               write(dunit,*)
            end do
         end do
 ...
 !-----------------------------------------------------------------------------------------!
 ! NOTE that B_adjust may want to be customised for the problem being
tackled.
 ! This version ought to work in general but uses ALL atoms which might
(a) be slow and
 ! (b) simply be not necessary and (c) some might be dummy atoms whose
Biso is not really
 ! relevant.  At the very least you need to set B_aim (in MC input file) 
to be the
 ! B-factor you want to aim for.  Recall that B = 8*(pi^2)*U.
 ! The core of this routine is stolen from diffuse.f [1], so it should
give Biso
 ...
 subroutine  B_adjust(B_aim)
   ...
   real*8                ::
sumx,sumy,sumz,sumua,sumub,sumuc,sumaa,sumab,sumac
         ...
         do il = 1, num_loc
            do iat = 1,num_at(iz)
               ...
               do  ic=1,simsize(3)
                  do  ib=1,simsize(2)
                     do  ia=1,simsize(1)
                        if((mocc(ia,ib,ic,il).eq.im).and.(zocc(ia,ib,ic,il).eq.iz))
then
 ...
 subroutine incupdate()
   ...
   !---------------------------------------------------------------------------------------------!
   ! This changes the widths of the increments put on variables in an MC
cycle.   This
   ! is to optimise the acceptance/rejection ratio.  It is a (fairly)
reliable routine
   ! but can act up under extreme circumstances.
   ! I am not sure how serious is the speed penalty of doing this; it may
well be best to use it
   ! to get a 'reasonable' set of increments then fix them for most of
the time.
   !---------------------------------------------------------------------------------------------!

   integer                                           :: iz, iivar

   real,allocatable                                  :: rejgrad(:),
gradnum(:),gradtot(:)
   real                                              :: tratio, tcyc
   ...
   ! is just a subtraction because in rise/run all have the same 'run' --
all
   ...
   do iz = 1,total_num_zm
      ...
      !-------------------------------------------------------------------------!
      ! 20 March 2007:  If not enough cycles done on that z-mat, skip
over it...
      !-------------------------------------------------------------------------!
      ...
      if(tcyc.gt.400.0) then
         ...
         !---------------------------------------------------------------------------------!
         ! Step 2
         ! We want all the variables to have the same impact on the
acc/rej ratio.
         ! on other words, we want all the values in rejarray() to be the
same.
         ! We also want the total rejection fraction to be around 50%.
         ! The first of these considerations scales the widths() and
inwidths() relative
         ...
         ! that variables with big gradients will have their effects
reduced:
         ! But we need some safeguards to stop things blowing up.
         ! If a width is less than 0.000005, we assume it is zero and is
meant
         ...
         do iivar = 1,7
            if(widths(iivar,iz).gt.0.0000) then
         ...
         do iivar = 1, nintern(iz)
            if(inwidths(iivar,iz).gt.0.0000) then
 ...
 subroutine get_info(outname,outf)
   ...
   !----------------------------------------------------------------------------------------------
   !  This prints out some useful guff about the end simulation. 
Histograms, mostly.
   !----------------------------------------------------------------------------------------------
   ...
   integer                                       :: ia2, ib2, ic2,
typ,k,ddm,ddz,nbin,ibin
   integer                                       ::
oa,ob,oc,oz,ol,om,stdou2,i,j
...
count_in(:,:),total_val(:,:),total_val_sq(:,:)
      ...
      do ob = 3,simsize(2)-2
         do oc = 3,simsize(3)-2
            do ol = 1,num_loc
               ...
               oz = zocc(oa,ob,oc,ol)
               if
(total_num_zm.lt.zocc(oa,ob,oc,ol))total_num_zm=zocc(oa,ob,oc,ol)
               do k = 1,conmol(ol,om,oz)
                  ia2 = wrap(oa+da(ol,oz,om,k),1)
                  ib2 = wrap(ob+db(ol,oz,om,k),2)
                  ic2 = wrap(oc+dc(ol,oz,om,k),3)
                  ddz = zocc(ia2,ib2,ic2,dl(ol,oz,om,k))
                  ddm = mocc(ia2,ib2,ic2,dl(ol,oz,om,k))
                  if( (ddz.eq.dz(ol,oz,om,k)).and.
(ddm.eq.dm(ol,oz,om,k)) ) then
                     x2 = carts(1,oa,ob,oc,ol,oat(ol,oz,om,k))
                     y2 = carts(2,oa,ob,oc,ol,oat(ol,oz,om,k))
                     z2 = carts(3,oa,ob,oc,ol,oat(ol,oz,om,k))
                     xyz(1) =
carts(1,ia2,ib2,ic2,dl(ol,oz,om,k),dat(ol,oz,om,k))
                     xyz(2) =
carts(2,ia2,ib2,ic2,dl(ol,oz,om,k),dat(ol,oz,om,k))
                     xyz(3) =
carts(3,ia2,ib2,ic2,dl(ol,oz,om,k),dat(ol,oz,om,k))
                     xyz =
xyz+da(ol,oz,om,k)*celltrans(1,:)+db(ol,oz,om,k)*celltrans(2,:)+
&
                          dc(ol,oz,om,k)*celltrans(3,:)
...
sqrt((xyz(1)-x2)*(xyz(1)-x2)+(xyz(2)-y2)*(xyz(2)-y2)+(xyz(3)-z2)*(xyz(3)-z2))
   ...
   if (.not. quiet)write(stdout,*)'Writing out global averages for
contact vectors...'
   write(stdou2,*)'----------------------------------------------------------------------------'
   write(stdou2,*)'Average contact vector lengths across the whole
simulation...'
   write(stdou2,*)'Contact type, number, average length, shortest,
longest, stdev'
   do k = 1,ntyp
...
sqrt((xyz(1)-x2)*(xyz(1)-x2)+(xyz(2)-y2)*(xyz(2)-y2)+(xyz(3)-z2)*(xyz(3)-z2))
                     do ibin = 1, nbin
                        if( (
dist.gt.(shortest(typ)+(real(ibin-1))*((longest(typ)-shortest(typ))/real(nbin)))
)    &
                             .and.(dist.lt.(shortest(typ)+(real( ibin
))*((longest(typ)-shortest(typ))/real(nbin)))))
  &
   ...
   if (.not. quiet)write(stdout,*)'Writing out histograms for contact
vectors...'
   do k = 1,ntyp
      write(stdou2,*)'----------------------------------------------------------------------------'
         ...
         do oc = 1,simsize(1)
            do ol = 1,num_loc
               ...
               do typ = 1,nintern(oz)
                  ...
                  total_val(oz,typ) = total_val(oz,typ) +
incrystal(oa,ob,oc,ol,typ)
                  total_val_sq(oz,typ) = total_val_sq(oz,typ) +
(incrystal(oa,ob,oc,ol,typ))**2.0
...
internal degrees of freedom...'
   write(stdou2,*)'----------------------------------------------------------------------------'
   write(stdou2,*)'Average values of internals across the whole
simulation...'
         ...
         do oc = 1,simsize(1)
            do ol = 1,num_loc
               ...
               do typ = 1,nintern(oz)
                  ...
                  do ibin = 1, nbin
                     if( (
dist.gt.(short_in(oz,typ)+(real(ibin-1))*((long_in(oz,typ)-short_in(oz,typ))/real(nbin)))
)    &
  ...
  &
   ...
   if (.not. quiet)write(stdout,*)'Writing out histograms for internal
degrees of freedom...'
   do oz = 1,total_num_zm
      do k = 1,nintern(oz)
         write(stdou2,*)'----------------------------------------------------------------------------'
...
iia,iib,iic,iil,iz,i,dunit
   ...
   if (ierr.ne.0) call allerr('interns(maxval(nintern))                  
        ')
   if (.not. quiet)write(stdout,*)'Model crystal to be read in from
',trim(inname)
   if(inname.eq.outname) then
      if (.not. quiet)write(stdout,*)'Note: If performing MC, file will
be overwritten unless number of MC cycles is zero!!!!!'
   end if
   ...
   do ia = 1,simsize(1)
      do ib = 1,simsize(2)
         do ic = 1,simsize(3)
            do il = 1,num_loc
               read(dunit,'(4i3,1x,7(f8.5,1x),L3,1x)', advance =
'no',iostat=ierr)iia,iib,iic,iil,  &
                    trans(:),ddd(:),improp
               if(ierr.ne.0) then
                  write(stderr,*)'----------------------------------------------------------------------------'
                  write(stderr,*)'Problem reading in model crystal
file:',trim(inname)
               ...
               do i = 1,nintern(iz)
                  read(dunit,'(f8.5,1x)',
advance='no',iostat=ierr)interns(i)
                  if(ierr.ne.0) then
                     write(stderr,*)'----------------------------------------------------------------------------'
                     write(stderr,*)'Problem reading in model crystal
file:',trim(inname)
               ...
               !----------------------------------------------------------------
               ! Now we have read in a single row of the file. Put it
into arrays
               !----------------------------------------------------------------
...
ia,ib,ic,il,im,iz,j,dunit,i
   ...
   if (.not.
quiet)write(stdout,*)'----------------------------------------------------------------------------'
   if (.not. quiet)write(stdout,*)'Writing Cartesian coordinates to
',trim(xyzfile)
   if((addcelltrans.eq.'y').or.(addcelltrans.eq.'Y')) then
      if (.not. quiet)write(stdout,*)'Coordinates will include unit cell
translations'
   else
      if (.not. quiet)write(stdout,*)'Coordinates will not include unit
cell translations'
   end if

   dunit = open(xyzfile)

   write(dunit,*)'ia  ib  ic  il  iz  im   iat         x         y       
z'
...
cvec,fvec
   ...
   type (crystallography_object),intent(in)                           ::
xtal

   if (.not.
quiet)write(stdout,*)'----------------------------------------------------------------------------'
   if (.not. quiet)write(stdout,*)'Writing fractional coordinates to
',trim(xyzfile)
   dunit = open(xyzfile)
...
z'
 ...
 subroutine peanut(outname,oora)
   ...
   !----------------------------------------------------------------
   ! Produces correlation diagrams which plot correlation in the
displacements of
   ! molecular origins or average positions across atoms of contacting
molecules
   ! as a function of direction of displacement in each plane ab, ac, bc...
   ! Hence there will be three diagrams for for each contact vector type
-- note
   ! that there will be redundancy as some contact vectors will join the
same pairs
   ...
   character(len=2)                              :: kip
   character(len=110)               ,dimension(3):: xyzfile
   character(len=100), intent(in)                :: outname

   integer                                             :: ia2, ib2, ic2,
typ,k,ddm,ddz,otyp
   integer                                             ::
oa,ob,oc,om,oz,ol,icorr_angle,stdou2
   integer                                             :: stdou1,stdou3
   integer                                             ::
ia,ib,ic,im,iz,il,i,j,iflag
...
avgcount,corr_angle
   real                                                               ::
unit_x,unit_y,ovec_proj
   real                                                               ::
dvec_proj,pi,corr1,corr2
   real                                                               ::
corr3,corr
   double precision,dimension(360,3)                                  ::
sumxy360,sumxx360
   double precision                                                   ::
n360
   double precision,dimension(360,3)                                  ::
sumyy360,sumx360,sumy360

   !----------------------------------------------------------------
   ! First, to do the peanut diagrams we are going to use the position of
the origin atom
   ! or the average of all atoms as the position of the molecule --
depends on command line.
   !----------------------------------------------------------------

   allocate(globeave(3,num_loc,total_num_zm,maxval(mocc)),stat=ierr)
   if (ierr.ne.0) call
allerr('globeave(3,num_loc,total_num_zm,maxval(mocc))       ')
   allocate(globeavecount(num_loc,total_num_zm,maxval(mocc)),stat=ierr)
   if (ierr.ne.0) call
allerr('globeavecount(num_loc,total_num_zm,maxval(mocc))    ')
   ...
   if (.not. quiet) then
      ...
      write(stdout,*)'Note that the planes are Cartesian, and do NOT
relate directly to the'
      write(stdout,*)'real-space lattice planes in systems of less than
orthorhombic symmetry.'
      write(stdout,*)'xy plane should be OK regardless.'
      write(stdout,*)
      write(stdout,*)'Files will have names of the form
r_NUM_OA_XY_outname.out'
      write(stdout,*)'where NUM is the contact vector type and OA is o or
a for origin or'
      write(stdout,*)'average. XY is the plane. (xy, yz, zx).  outname is
the name specified in MC file'
      write(stdout,*)
      write(stdout,*)'To plot these numbers, fire up gnuplot and, if you
are in the same directory'
      write(stdout,*)'as the output files, type (at the prompt,
"gnuplot>") something like:'
      write(stdout,*)'gnuplot> set angles degrees; set polar; plot
"r_01_o_xy_outname.out" '
   ...
   if ((oora.eq.'o').or.(oora.eq.'O')) then
      ...
      !----------------------------------------------------------------
      ! Extract origin and put it in working array.  Need to get the
'global'
            ...
            do ic = 1,simsize(3)
               do il = 1,num_loc
                  ...
                  do j = 1,3
                     ...
                     globeave(j,il,iz,im) = globeave(j,il,iz,im)+
carts(j,ia,ib,ic,il,1)
                  end do
               end do
            end do
         end do
      ...
      !----------------------------------------------------------------
      ! Extract average and put that in the working array.  Note: we ARE
using dummy atoms!!!
      !----------------------------------------------------------------
...
carts(j,ia,ib,ic,il,i)
   ...
   !----------------------------------------------------------------
   ! OK, now we should have a single atom's coordinates in the array
working-array(), which will
   ! be the positions of the origin atom OR the average of all in the
molecule, including dummies
   ! We also have the global average position of the origin of each
molecule in globeave()

   ! First, we select an origin molecule.  We'll sum over
   ! an inner subset of the crystal in order to remove the need for
wrapping arrays.
         ...
         do ob = 2,(simsize(2)-2),2
            do oc = 2,(simsize(3)-2),2
               do ol = 1,num_loc
                  ...
                  ! Now, get the first contact vector coming off this
molecule
                  !-------------------------------------------------
                  ...
                  do k = 1,conmol(ol,om,oz)
                     if (iflag.eq.0) then
                        ...
                        if(typ.eq.otyp) then
                           ...
                           if((oa.eq.ia2).and.(ob.eq.ib2).and.(oc.eq.ic2))then
...
(ddm.eq.dm(ol,oz,om,k)) ) then
                                 ...
                                 !----------------------------------
                                 ! Then subtract from these their
'average' positions:
                                 !----------------------------------
                                 ...
                                 do j=1,3
                                    ...
                                    if(globeave(j,ol,oz,om).ge.99999.0) then
                                       write(stderr,*)'Problem with
global average.  Code GLOB1'
                                       STOP
                                    end if
                                    if(globeave(j,dl(ol,oz,om,k),ddz,ddm).ge.99999.0)
then
                                       write(stderr,*)'Problem with
global average.  Code GLOB2'
                                       STOP
                                    end if
                                 end do

                                 !----------------------------------
                                 ! So now we have two vectors displaced
relative to their
                                 ! respective 'origins'.
                                 ! Now decompose them.
                                 ! Use a Cartesian set of coordinates
                                 ! along x, y and z.  NOTE that this is NOT
                                 ! the crystal axes in a low symmetry
system.
                                 ! Of course in fact they are already
decomposed
                                 ...
                                 do icorr_angle = 1,360
                                    ...
                                    !----------------------------------
                                    ! Components of unit vector in
direction corr_angle is:
                                    !----------------------------------
...
displacement onto this direction is:
                                    ! (Note, must keep the sign....)
                                    !----------------------------------

                                    ovec_proj =
ovec(1)*unit_x+ovec(2)*unit_y
                                    dvec_proj =
dvec(1)*unit_x+dvec(2)*unit_y
                                    ...
                                    sumxy360(icorr_angle,1) =
sumxy360(icorr_angle,1) +
ovec_proj*dvec_proj
                                    sumxx360(icorr_angle,1) =
sumxx360(icorr_angle,1) +
ovec_proj**2
                                    sumyy360(icorr_angle,1) =
sumyy360(icorr_angle,1) +
dvec_proj**2
                                    sumx360(icorr_angle,1)  =
sumx360(icorr_angle,1)  + ovec_proj
                                    sumy360(icorr_angle,1)  =
sumy360(icorr_angle,1)  + dvec_proj
                                 ...
                                 do icorr_angle = 1,360
                                    ...
                                    unit_y = sin(corr_angle)
                                    ovec_proj =
ovec(2)*unit_x+ovec(3)*unit_y
                                    dvec_proj =
dvec(2)*unit_x+dvec(3)*unit_y
                                 ...
                                 do icorr_angle = 1,360
                                    ...
                                    unit_y = sin(corr_angle)
                                    ovec_proj =
ovec(3)*unit_x+ovec(1)*unit_y
                                    dvec_proj =
dvec(3)*unit_x+dvec(1)*unit_y
      ...
      if(otyp.lt.10)write(kip,'(a1,i1.1)')'0',otyp
      if(otyp.ge.10)write(kip,'(i2.2)')otyp

      write(xyzfile(1),'(a2,a2,a1,a1,a4,a100)')'r_',kip,'_',oora,'_xy_',outname
      write(xyzfile(2),'(a2,a2,a1,a1,a4,a100)')'r_',kip,'_',oora,'_yz_',outname
      write(xyzfile(3),'(a2,a2,a1,a1,a4,a100)')'r_',kip,'_',oora,'_zx_',outname
      ...
      do icorr_angle = 1,360
         corr1 =
sumxy360(icorr_angle,1)-(sumx360(icorr_angle,1)*sumy360(icorr_angle,1))/n360
      ...
      do icorr_angle = 1,360
         corr1 =
n360*sumxy360(icorr_angle,2)-sumx360(icorr_angle,2)*sumy360(icorr_angle,2)
         corr2 =
n360*sumxx360(icorr_angle,2)-sumx360(icorr_angle,2)*sumx360(icorr_angle,2)
         corr3 =
n360*sumyy360(icorr_angle,2)-sumy360(icorr_angle,2)*sumy360(icorr_angle,2)
         corr = corr1/sqrt(corr2*corr3)
      ...
      do icorr_angle = 1,360
         corr1 =
n360*sumxy360(icorr_angle,3)-sumx360(icorr_angle,3)*sumy360(icorr_angle,3)
         corr2 =
n360*sumxx360(icorr_angle,3)-sumx360(icorr_angle,3)*sumx360(icorr_angle,3)
         corr3 =
n360*sumyy360(icorr_angle,3)-sumy360(icorr_angle,3)*sumy360(icorr_angle,3)
         corr = corr1/sqrt(corr2*corr3)
 ...
 subroutine pairs(outname)
   ...
   !--------------------------------------------------------------------------
   ! This simply puts out to files the variables for pairs of molecules
separated by               !
   ! each contact vector.  These can then be plotted, correlations looked
for etc.                !
   !--------------------------------------------------------------------------

   character(len=2)                              :: kip
   character(len=111)                            :: pairfile
   character(len=100),intent(in)                 :: outname
...
typ,k,ddm,ddz,otyp
   ...
   if(.not.quiet) then
      write(stdout,*)'----------------------------------------------------------------------------'
      write(stdout,*)'Running subroutine to put out the molecular
variables (x, q, i)'
      write(stdout,*)'for pairs of molecules connected by each type of
contact vector.'
      write(stdout,*)'The columns of numbers (a subset of the full
crystal) can then be'
      write(stdout,*)'imported into various programs (R, gnuplot,
gnumeric, Excel).'
      write(stdout,*)'It assumes that a given contact vector always has
the same types'
      write(stdout,*)'of z-matrices at its ends.  If this is not the
case, interpret with care!'
      write(stdout,*)
      write(stdout,*)'Files will have names of the form
pairs_X_NUM_outname.out'
   ...
   do otyp = 1,ntyp
      ...
      do ol = 1,num_loc
         iiflag = 0
         write(pairfile,'(a6,i1,a1,a2,a1,a100)')'pairs_',ol,'_',kip,'_',outname
         dunit = open(pairfile)
         do oa = 2,(simsize(1)-2),2
            do ob = 2,(simsize(2)-2),2
               do oc = 2,(simsize(3)-2),2
...
(ddm.eq.dm(ol,oz,om,k)) ) then
                                 ...
                                 ! Write out the pair oa,ob,oc,ol and
ia2,ib2,ic2,dl(ol,oz,om,k)

                                 q2 = qcrystal(ia2,ib2,ic2,dl(ol,oz,om,k))
                                 do i = 1,3
                                    x2(i) =
xyzcrystal(ia2,ib2,ic2,dl(ol,oz,om,k),i)
                                 end do
                                 ...
                                 if (iiflag.eq.0)then
                                    iiflag = 1
                                    write(dunit,'(7a8," ",a3,"
")',advance='no')'x_1','y_1','z_1','q1_1','q1_2','q1_3','q1_4','im1'
                                    do i = 1,nintern(oz)
                                       write(dunit,'(a6,i2," ")',
advance='no')'intl1_',i
                                    end do
                                    write(dunit,'(7a8," ",a3,"
")',advance='no')'x_2','y_2','z_2','q2_1','q2_2','q2_3','q2_4','im2'
                                    do i = 1,nintern(ddz)
                                       write(dunit,'(a6,i2," ")',
advance='no')'intl2_',i
                                    end do
                                 ...
                                 write(dunit,'(7f8.5," ",L3,"
")',advance='no')(x1(i),i=1,3),(as_array(q1)),improper(q1)
                                 do i = 1,nintern(oz)
                                    write(dunit,'(f8.5," ")',
advance='no')incrystal(oa,ob,oc,ol,i)
                                 end do
                                 write(dunit,'(7f8.5," ",L3,"
")',advance='no')(x2(i),i=1,3),(as_array(q2)),improper(q2)
                                 do i = 1,nintern(ddz)
                                    write(dunit,'(f8.5," ")',
advance='no')incrystal(ia2,ib2,ic2,dl(ol,oz,om,k),i)
                                 end do
                                 write(dunit,*)
 ...
 subroutine calc_uij_new(xtal,outname)
   ...
   integer                                       ::
ia,ib,ic,im,iz,il,i,j,stdou2,inum
   ...
   if (.not.
quiet)write(stdout,*)'----------------------------------------------------------------------------'
   if (.not. quiet)write(stdout,*)'Routine to output a .cif file; the
file should be readable by Mercury if '
   if (.not. quiet)write(stdout,*)'nothing else, since that was used to
test the routine.      '
      ...
      do ib = 1,simsize(2)
         do ic = 1,simsize(3)
            do il = 1,num_loc
               ...
               do i = 1,num_at(iz)
                  do j = 1,3
                     globeave(j,il,iz,im,i) = globeave(j,il,iz,im,i) +
carts(j,ia,ib,ic,il,i)
                  end do
               end do
            end do
...
globeavecount(il,iz,im)
   ...
   ! and in the a*a coords used to define the Uij the means are in
globeave2()
         ...
         do ic = 1,simsize(3)
            do il = 1,num_loc
               ...
               do i = 1,num_at(iz)
                  ...
                  fvec(:) = as_fractional(xtal,ovec(:))
                  do j = 1,3
                     ovec(j) = fvec(j) * cellaxes(j) -
globeave2(j,il,iz,im,i)
                  end do
      ...
      do iz = 1, total_num_zm
         do im = 1,maxval(mocc)
            if(globeavecount(il,iz,im).gt.0.5) then
               do i = 1,num_at(iz)
                  do j = 1,6
                     uij(il,iz,im,i,j) = uij(il,iz,im,i,j) /
globeavecount(il,iz,im)
                  end do
               end do
            end if
   ...
   if (.not. quiet)write(stdout,*)'Spacegroup will be written as P1, .cif
file is ',trim(xyzfile)
   stdou2 = open(xyzfile)
   write(stdou2,'(a)')'###########################################################################'
   write(stdou2,'(a)')'#                                                 
                      #'
   write(stdou2,'(a)')'#                 CIF from ZMC, version as of
August 19, 2008             #'
   write(stdou2,'(a)')'#                                                 
                      #'
   write(stdou2,'(a)')'###########################################################################'
   write(stdou2,'(a)')'#                                                 
                      #'
   write(stdou2,'(a)')'#  This CIF contains data generated by averaging
across a model crystal   #'
   write(stdou2,'(a)')'#  as used in the program ZMC to model short-range
order in crystalline   #'
   write(stdou2,'(a)')'#  materials.  The program assumes NO symmetry, so
ALL sites in the       #'
   write(stdou2,'(a)')'#  unit cell are in the file, and space group is
given as P1.             #'
   write(stdou2,'(a)')'#                                                 
                      #'
   write(stdou2,'(a)')'#  The routine keeps the first two characters of
the atom label from      #'
   write(stdou2,'(a)')'#  the z-matrix and adds the ordinal number of the
atom in the unit cell. #'
   write(stdou2,'(a)')'#                                                 
                      #'
   write(stdou2,'(a)')'#  This CIF generation routine has been tested by
importing the output    #'
   write(stdou2,'(a)')'#  files into Mercury
(http://www.ccdc.cam.ac.uk/products/mercury/)       #'
   write(stdou2,'(a)')'#  version 1.4.2 (Build 2).  No guarantees on the
quality of the CIF      #'
   write(stdou2,'(a)')'#  files generated are given or implied.          
                      #'
   write(stdou2,'(a)')'#                                                 
                      #'
   write(stdou2,'(a)')'#  For further information about ZMC, contact
goossens@rsc.anu.ed.au      #'
   write(stdou2,'(a)')'#                                                 
                      #'
   write(stdou2,'(a)')'###########################################################################'
            ...
            if(globeavecount(il,iz,im).gt.0.5) then
               do i = 1, num_at(iz)
                  ...
                  if (len_trim(lable).ge.2) then
                     if (inum.le.9)
write(lablefinal,'(a2,i1)')lableshort,inum
                     if
((inum.ge.10).and.(inum.le.99))write(lablefinal,'(a2,i2)')lableshort,inum
                     if
((inum.ge.100).and.(inum.le.999))write(lablefinal,'(a2,i3)')lableshort,inum
                     if
((inum.ge.1000).and.(inum.le.9999))write(lablefinal,'(a2,i4)')lableshort,inum
                     if
((inum.ge.10000).and.(inum.le.99999))write(lablefinal,'(a2,i5)')lableshort,inum
                     if
((inum.ge.100000).and.(inum.le.999999))write(lablefinal,'(a2,i6)')lableshort,inum
                     if
((inum.ge.1000000).and.(inum.le.9999999))write(lablefinal,'(a2,i7)')lableshort,inum
                  else
                     if (inum.le.9)
write(lablefinal,'(a1,i1)')lableshort,inum
                     if
((inum.ge.10).and.(inum.le.99))write(lablefinal,'(a1,i2)')lableshort,inum
                     if
((inum.ge.100).and.(inum.le.999))write(lablefinal,'(a1,i3)')lableshort,inum
                     if
((inum.ge.1000).and.(inum.le.9999))write(lablefinal,'(a1,i4)')lableshort,inum
                     if
((inum.ge.10000).and.(inum.le.99999))write(lablefinal,'(a1,i5)')lableshort,inum
                     if
((inum.ge.100000).and.(inum.le.999999))write(lablefinal,'(a1,i6)')lableshort,inum
                     if
((inum.ge.1000000).and.(inum.le.9999999))write(lablefinal,'(a1,i7)')lableshort,inum
                  end if
                  write(stdou2,'(a10,4f9.5,a5)')lablefinal,(fvec(j),j=1,3),Uiso,'
Uani'
               end do
            end if
         end do
...
((inum.ge.1000000).and.(inum.le.9999999))write(lablefinal,'(a1,i7)')lableshort,inum
                  ...
                  write(stdou2,'(a10,6f10.6)')lablefinal,
(uij(il,iz,im,i,j),j=1,6)
               end do
            end if
         end do
      end do
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 subroutine usage()
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| zmc [--option_1] [--option_2] ... [--option_n]
infile [outfile]     |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| infile contains the parameters and additional
filenames to run      |'
   write(stdout,*)'| the MC simulation.                                  
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| outfile is the root name for most output; if not
given the root     |'
   write(stdout,*)'| name will be infile (i.e., outfile = infile)        
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| Options always begin with two dashes, and if the
option can be      |'
   write(stdout,*)'| passed a value, the value must be indicated with an
equals sign     |'
   write(stdout,*)'| and there must be no spaces.  E.g.: --summary=inline
is right,      |'
   write(stdout,*)'| "--summary inline" is wrong, "--summary = inline" is
wrong.         |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| Items enclosed in square brackets are optional.     
              |'
   write(stdout,*)'|                                                     
              |'
   ...
   write(stdout,*)'| --crystal[=filename]                                
              |'
   write(stdout,*)'|           Causes program to output a text file
referred to as a     |'
   write(stdout,*)'|           "ZMC crystal file" which contains the
variables for each  |'
   write(stdout,*)'|           molecule.  Can be analysed and/or read
back in via        |'
   write(stdout,*)'|           --reread[=filename] to resume simulation. 
If filename    |'
   ...
   write(stdout,*)'| --diffuse[=filename]                                
              |'
   write(stdout,*)'|           Causes program to output a file designed
to be read into  |'
   write(stdout,*)'|           diffuse scattering calculation program
DIFFUSE.           |'
   write(stdout,*)'|           Butler and  Welberry J.Appl.Cryst.(1992)25
391-399        |'
   write(stdout,*)'|           If filename not given, will write to
outfile.diffuse.     |'
   write(stdout,*)'|           If outfile not given, will write to
infile.diffuse        |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --summary[=inline]                                  
              |'
   write(stdout,*)'|           Prints out some information about the
model crystal at    |'
   write(stdout,*)'|           the end of the simulation.  If no argument
passed, sends  |'
   write(stdout,*)'|           output to outfile.summary. inline writes
output to stdout.|'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --pairs   Outputs variables for pairs of molecules
connected by     |'
   write(stdout,*)'|           each contact vector to files
pairs_L_C_outfile.out        |'
   write(stdout,*)'|           where L is the location number of the
origin molecule     |'
   write(stdout,*)'|           C is contact number.  Many of these files
will be         |'
   write(stdout,*)'|           duplicates.                               
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --fracs   Outputs fractional coordinates of every
atom in the model |'
   write(stdout,*)'|           crystal to outfile.fracs                  
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --cartsn  Outputs Cartesian coordinates of every
atom in the model  |'
   write(stdout,*)'|           to outfile.cartsn. Does not add on unit
cell translations.|'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --cartst  Outputs Cartesian coordinates of every
atom in the model  |'
   write(stdout,*)'|           to outfile.cartst. Adds on unit cell
translations.        |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --reread=filename                                   
              |'
   write(stdout,*)'|           Reads in a crystal file and uses it as
starting point for !'
   write(stdout,*)'|           the simulation.  Reads in from filename.  
              !'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --corro   Calculates correlation ("peanut") diagrams
and writes     |'
   write(stdout,*)'|           them to files r_C_o_XX_outname.out where C
is the         |'
   write(stdout,*)'|           contact vector and XX is the plane (xy,
yz, zx).          !'
   write(stdout,*)'|           Uses the position of the origin atom of
each molecule     !'
   write(stdout,*)'|           in the calculation.  Many of these files
will be          |'
   write(stdout,*)'|           duplicates.                               
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --corra   As for --corro except file names are
r_C_a_XX_outname.out |'
   write(stdout,*)'|           and it uses the average over all atoms in
the molecule.   |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --energy  Outputs the MC energy of each molecule to
outfile.energy  |'
   write(stdout,*)'|                                                     
              |'
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --quiet   ZMC sends nothing to the screen except
error messages and |'
   write(stdout,*)'|           summary information if --summary=inline is
set.           |'
   write(stdout,*)'|                                                     
              |'
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --plot    Runs interactive plotting of the
simulation; crude but    |'
   write(stdout,*)'|           sometimes useful. Exits after producing
plots.            |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --getcontacts                                       
              |'
   write(stdout,*)'|           Runs a simple routine to generate contact
vectors         |'
   write(stdout,*)'|           based on parameters specified in keyword
file.            |'
   write(stdout,*)'|           Then exits.                               
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| --help, --help2 and --version can be run without
infile             |'
   write(stdout,*)'| or outfile.out being specified. --quiet does not
work with --help,  |'
   write(stdout,*)'| --help2 or --version. If --getcontacts is given,
will not do MC     |'
   write(stdout,*)'| or plot.  If --plot is given, will not proceed to
MC.               |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'|---------------------------------------------------------------------|'
...
20  if(iend.lt.1) then
 ...
 subroutine plot(con_test)
   ...
   character(len=1)          ::
nmal,co1,co2,pcon,numatom,writenum,labcon,plmult
   ...
   integer                   :: icon,
ii,mm,iil,iiz,im,ia,ib,ic,ia2,ib2,ic2,iiat,ranflag,slthick
...
1191 write(stdout,*)'How thick (in unit cells) for slice down view dirn  '
   call read_buffer(stdin, buffer, finished, comment="!#",
removeblanks=.TRUE.)
...
111 write(stdout,*)'Plot contacts y or n'
   ...
   if(con_test.eq.-1) then
      pcon = 'n'
      write(stdout,*)'No valid contact vector list specified. Not
plotting contacts!'
   end if
...
removeblanks=.TRUE.)
         ...
         if
((minval(pltypes(1:nummult)).lt.1).or.(ierr.ne.0).or.(maxval(pltypes).gt.ntyp))
then
...
quiet)write(stdout,*)'-------------------------------------------------------------------------------'
   ...
   !We can view the crystal down one of the three real-space cartesian
directions (NOT lattice vectors)
   view(1) = 'x'
   view(2) = 'y'
   view(3) = 'z'
   f2name = 'courier'
6546 FORMAT(4i2)
   do icon = 1,ntyp
      do ii = 1,3
         ...
         !-------------------------------------------------------------------------------------!
         ! Initialise the plot page.  In ps_init (ps_routines.f) I have
changed the length of
         ...
         DO iz = 1,total_num_zm
            ...
            testzmat = zmat(iz)
            conmax(iz) =
maxval(parameter(testzmat,(/(i,i=2,mm)/),(/(1,i=2,mm)/)))+0.3
            if(conmax(iz).gt.1.8)conmax(iz)=1.8
            conmin(iz) =
minval(parameter(testzmat,(/(i,i=2,mm)/),(/(1,i=2,mm)/)))
         end do
         ...
         DO ia = 1,si(1)
            DO ib = 1,si(2)
               DO ic = 1,si(3)
                  cvec(:) =
as_cartesian(xtal,real((/(ia-1),(ib-1),(ic-1)/)))
                  DO iil = 1,num_loc
                     ! Now cycle over each molecule within a type (with
the same z-matrix)
                     iiz = zocc(ia,ib,ic,iil)
                     im = mocc(ia,ib,ic,iil)
                     call
ps_rgbcolor(c1(mod(iil,35)),c2(mod(iil,35)),c3(mod(iil,35)))
                     DO iat =1, num_at(iiz)
                        IF (nmal.eq.'z') then
                        ...
                        ! This bit draws in the intra molecular bonds --
not contacts
                        DO iiat = 1,num_at(iiz)
                           ...
                           END IF
                           dx = carts(1,ia,ib,ic,iil,iat)-
carts(1,ia,ib,ic,iil,iiat)
                           dy = carts(2,ia,ib,ic,iil,iat)-
carts(2,ia,ib,ic,iil,iiat)
                           dzz = carts(3,ia,ib,ic,iil,iat)-
carts(3,ia,ib,ic,iil,iiat)
                           dist = sqrt(dx**2+dy**2+dzz**2)
                           ! If bond is in right range, draw it.
                           if
((dist.ge.conmin(iiz)-0.005).and.(dist.le.conmax(iiz)+0.005))
 &
                                call ps_line(xxx,yyy,xxxx,yyyy)
                        END DO  !iiat
                     END DO   !iat
                     ! This bit writes unit cell and mol number next to
each molecule.
                     call ps_rgbcolor(0.,0.,0.)
                     ...
                     ! This is where we are drawing contacts rather than
just molecules
                     if((pcon.eq.'n').or. (pcon.eq.'N')) then
                        ...
                        do k = 1,conmol(iil,im,iiz)
                           pltest = 0
                           if((ty(iil,iiz,im,k).eq.icon).and.(plmult.eq.'n'))
pltest = 2345
                           if((any(pltypes(1:nummult).eq.ty(iil,iiz,im,k))).and.(plmult.eq.'y'))
pltest = 2345
                           if (pltest.eq.2345) then
                              pltest = 0
                              x2 =
carts(1,ia,ib,ic,iil,oat(iil,iiz,im,k))+cvec(1)
                              y2 =
carts(2,ia,ib,ic,iil,oat(iil,iiz,im,k))+cvec(2)
                              z2 =
carts(3,ia,ib,ic,iil,oat(iil,iiz,im,k))+cvec(3)
                              ...
                              ! See what is actually there at the end of
the contact we are plotting
                              if((zocc(ia2,ib2,ic2,dl(iil,iiz,im,k)).ne.dz(iil,iiz,im,k)).or.
    &
                                   (mocc(ia2,ib2,ic2,dl(iil,iiz,im,k)).ne.dm(iil,iiz,im,k)))
goto 9087
                              cvec2(:) =
as_cartesian(xtal,real((/(ia2-1),(ib2-1),(ic2-1)/)))
                              x3 =
carts(1,ia2,ib2,ic2,dl(iil,iiz,im,k),dat(iil,iiz,im,k))+cvec2(1)
                              y3 =
carts(2,ia2,ib2,ic2,dl(iil,iiz,im,k),dat(iil,iiz,im,k))+cvec2(2)
                              z3 =
carts(3,ia2,ib2,ic2,dl(iil,iiz,im,k),dat(iil,iiz,im,k))+cvec2(3)
                              IF (nmal.eq.'z') then
                                 x2a = x2
                                 y2a = y2
                                 x3a = x3
                              ...
                              END IF
                              dist =
sqrt((x3-x2)**2.0+(y3-y2)**2.0+(z3-z2)**2.0)
                              ! use this as a test compare with expected
length
                              if(abs(dist-length(iil,iiz,im,k)).gt.0.005)
ranflag = 99999
                              call ps_rgbcolor(0.,0.,0.)
                              ...
                              if(labcon.eq.'y') then
                                 ...
                                 call
ps_string((x2a+x3a)/2.0,(y2a+y3a)/2.0,ctext,0.0,12)
                              end if
                              call ps_rgbcolor(0.,0.,0.)
...
9087                        fs=fs
         ...
         close(13)
         if (.not. quiet)write(stdout,*)
         if (.not. quiet)write(stdout,'(" Contact ",i3," viewed down
",a1," plotted to",a15)')icon,view(ii),psname
         if (.not.
quiet)write(stdout,*)'-------------------------------------------------------------------------------'
         if (ranflag.eq.99999)then
            if (.not. quiet)write(stdout,*)' Calculated and stored
contact lengths unequal.'
            if (.not. quiet)write(stdout,*)' If there is meant to be no
randomness in the  '
            if (.not. quiet)write(stdout,*)' model then this is a
problem...               '
            if (.not. quiet)write(stdout,*)'
-----------------------------------------------------------'
            ranflag = 0
 ...
 subroutine get_w(init_w,flag)
   ...
   !--------------------------------------------------------------------------
   ! First, test all occurances of keyword to see if any do not have a
character
   ...
   ! OK, there are three possibilities -- three fields follow the
keyword, two fields or one.
   ! If only one field, it is must be real and is the initw for xy & z
   ! If two fields and the first is x, y or z then the second must be a
real and
   ! must apply to all zmats
   !
   ! If two fields and the first is a integer, the second must be a real
that applies to that zmatrix.
   ! If three fields, the first should be an integer, the second a latter
and the third a real.
   ! So first, we want to work out which occurance of XYZINITWIDTH is
followed by a single real.
   !--------------------------------------------------------------------------
   ...
   if(numtimes.gt.1) then
      if(.not.quiet) write(stdout,*)trim(keyword),' default set more than
once.'
      if(.not.quiet) write(stdout,*)'Will use last occurance in keyword
file.'
   end if

   !--------------------------------------------------------------------------
   ! OK, now that the default is established, we look for cases with two
input fields.
   !--------------------------------------------------------------------------
   ...
   do i = 1,n
      if (fields_i(i).eq.2) then
         ...
         if(.isletter.trim(xyz)) then
            ! Then treat it as a global setting for a particular
coordinate, x, y or z
...
coordinate, I1...IN
            ...
            coord = 0
            ! Now here it gets tricky, because I don't know how many
components there
   ...
   if(.not.quiet) then
      ...
      do j = 1, total_num_zm
         ...
         write(stdout,'(1x,a7," widths for x,y,z are            
",3f9.5)') &
              prefix,(init_w(i,j),i=1,3)
         write(stdout,'(1x,a7," widths for q1,q2,q3,q4 are      
",4f9.5)') &
              prefix,(init_w(i,j),i=4,7)
         if(nintern(j).gt.0) then
            write(stdout,'(1x,a7," widths for internal d.f. are     
")',advance='no')prefix
            do k = 1,nintern(j)
               write(stdout,'(f8.5," ")', advance='no')init_w(k+7,j)
            end do
            write(stdout,*)
            write(stdout,*)'For internal d.f. that are angles, degrees
have been converted to radians.'
 ...
 subroutine get_num_fields(buffer, fields)
   ...
   comparator =
'/.,mnbvcxz;lkjhgfdsaA][poiuytrewq=-0987654321?><MNBVCXZ:LKJHGFDSA}{POIUYTREWQ+_)(*^%$#@!mdeoityrnba9'
   do j = 1,100
      test(j) = comparator
   end do
...
number of z-matrices.'
   ...
   do i = 1,total_num_qxyz
      ...
      if (.not. quiet)write(stdout,'(" Quaternions and translations for
z-matrix",i5)')zindex(i)
      dunit = open(qxyzname, status='old')
...
',trim(qxyzname)
         ...
         if(.not. quiet) then
            ...
            write(stdout,'(1x,"                   Quaternion: ",4F11.6)')
as_array(quatern(l,zindex(i),j))
            write(stdout,'(1x,"                     Improper: ",L4)') improp
            write(stdout,'(1x,"              COM Translation: ",3F11.6)')
translate(l,zindex(i),j,:)
         end if
...
file.'
   ...
   if (maxval_mocc.ne.-9999) then
      if(.not. quiet)write(stdout,*)'Interrogation of qxyz files suggests
there are'
      if(.not. quiet)write(stdout,'(" up to",i5," instances of a z-matrix
on a location.")')maxval_mocc
      if(.not. quiet)
write(stdout,*)'----------------------------------------------------------------------------'
   end if
...
cartscell(:,:,:,:,:)
   ...
   !----------------------------------------------------------------------------------------------
   ! OK, file is read in and opened.  now we need to find out which atoms
of which zmats to use,
   ...
   if (ierr.ne.0) call
allerr('which_atoms_to_use(total_num_zm,maxval(num_at))     ')
   ...
   do j = 1,num_value(key2,keyword)
      ...
      if (ierr.ne.0) call fatal_keyword(keyword, buffer)
      if (maxval(which_atoms_to_use(oz,:)).gt.num_at(oz)) call
fatal_keyword(keyword, buffer)
   end do
   ...
   !----------------------------------------------------------------------------------------------
   ! The first thing is to fill up an array of cartesian positions FOR A
SINGLE
   ! UNIT CELL.
   !----------------------------------------------------------------------------------------------
   ...
   do ol = 1,num_loc
      do iz = 1,how_many_types(ol)
         ...
         if (ierr.ne.0) call allerr('new_coords(3,(num_at(oz)))          
            ')
         call copy(z1,zmat(oz))
         do oom = 1,instances_types(ol,oz)
            om = which_mol(ol,oz,oom)
            ! So now we have defined the molecule of a certain z-matrix
type on a certain location.
            q = quatern(ol,oz,om)
            ...
            if(.not.quiet) then
               ...
               write(stdout,'(" Which is ordinal number : ",i5," on that
location.")')iz
               write(stdout,'(" In orientation number   : ",i5," for that
location.")')om
               do j = 1,num_at(oz)
      ...
      do iz = 1,how_many_types(ol)
         ...
         do oom = 1,instances_types(ol,oz)
            ...
            do iat = 1, num_atoms_to_use(oz)
               ...
               ! now we loop over neighbours looking for valid contact
vectors
               do da=-3,3,1
                  do db=-3,3,1
                     do dc=-3,3,1
                              ...
                              do iiz = 1,how_many_types(dl)
                                 ...
                                 do ddm = 1,instances_types(dl,dz)
                                    ...
                                    do iiat = 1,num_atoms_to_use(dz)
                                       ...
                                       ! separation, dding in unit cell
shift
                                       dest = dest + da*celltrans(1,:) +
db*celltrans(2,:) +
dc*celltrans(3,:)
                                       separation =
sqrt((dest(1)-origin(1))**2+     
&
                                            (dest(2)-origin(2))**2+      &
                                            (dest(3)-origin(3))**2)
                                       if((separation.gt.vmin).and.(separation.lt.vmax))
then
                                          ! write out the vector
                                          typ = typ + 1
                                          if(separation.lt.1.5) then
                                             if(.not.quiet) then
                                                write(stdout,*)'Contact
is less than 1.5 A!!'
                                                write(stdout,*)'Atom 1
',origin
                                                write(stdout,*)'Atom 2
 ...
 subroutine  get_required_real(value_given, keywrd)
   ...
   if (exists(key2, keywrd)) then
      if (has_value(key2, keywrd)) then
         ...
         if (.not. quiet)write(stdout,*)'Value for ',keywrd,' specified
explicitly is: ',value_given
         if (.not.
quiet)write(stdout,*)'----------------------------------------------------------------------------'
      else
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
         ...
         if(.not.quiet)write(stdout,*)'Keyword ',trim(keyword),' is
   ...
   if (exists(key2, keyword)) then
      if (has_value(key2, keyword)) then
         ...
         if(ierr.ne.0) then
            ...
            if(value_given.lt.1)then
               if (.not. quiet)write(stdout,*)'A value for ',keyword,' is
zero or negative.'
               if (.not. quiet)write(stdout,*)'Is this OK?'
               if (.not.
quiet)write(stdout,*)'----------------------------------------------------------------------------'
            ...
            if (.not. quiet)  &
                 write(stdout,'(" Values for ",a15," specified explicitly
are: ",i5,f9.5)')trim(keyword),value_given, real_val
            if (.not.
quiet)write(stdout,*)'----------------------------------------------------------------------------'
         end if
      else
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
      ...
      if (.not. quiet)write(stdout,*)'Values for ',keyword,' not
specified explicitly.'
      if (.not.
quiet)write(stdout,*)'----------------------------------------------------------------------------'
   end if
...
fatal_keyword(keyword,buffer)
            ...
            if (.not. quiet)write(stdout,'(" Number of cross terms for
z-matrix ",i5)')zmat_num(i)
            if (.not. quiet)write(stdout,'(" specified explicitly is:
",i5)')ncross_given(zmat_num(i))
            if (.not.
quiet)write(stdout,*)'----------------------------------------------------------------------------'
         end if
      end do ! i = 1,n
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 subroutine  get_sprcon_size(sprcon,keyword, ntyp)
   ...
   if((.not.quiet).and.(counter.gt.1))    &
        write(stdout,*)trim(keyword),' default set more than once.  Last
instance will be used.'
 ...
 subroutine get_inspr(insprcon, insprtype)
   ...
   do i = 1,n
      ...
      if(ierr.ne.0) call fatal_keyword(keyword,buffer)
      if((int_num.lt.1).or.(int_num.gt.nintern(z)))call
fatal_keyword(keyword,buffer)
      insprtype(int_num,z) = i
      if (.not.quiet) write(stdout,'("     Internal d.f. #",i5," of
z-matrix",i5)')int_num,z
      if (num_fields.gt.5) then
         ...
         do jj = 6, num_fields
            ...
            if (.isnumber. text) then
               ...
               insprtype(int_num,z) = i
               if (.not.quiet) write(stdout,'(" and internal d.f. #",i5,"
of z-matrix",i5)')int_num,z
               j = j + 1
               if(j.gt.num_fields)exit
            else
               ! It must be next ZMAT statement:
               text = .ucase.text
               if(trim(text).ne.trim(zmat)) call
fatal_keyword(keyword,buffer)
               j = j + 1
               ...
               j = j + 1
               if(ierr.ne.0) call fatal_keyword(keyword,buffer)
               if((z.lt.1).or.(z.gt.total_num_zm))call
fatal_keyword(keyword,buffer)
               ! then 'int'
               if(j.gt.num_fields)exit
               read(split_buffer(j),*,iostat=ierr) text
               ...
               if(ierr.ne.0) call fatal_keyword(keyword,buffer)
               text = .ucase.text
               if(trim(text).ne.trim(inte)) call
fatal_keyword(keyword,buffer)
               ! then int number
               if(j.gt.num_fields)exit
               read(split_buffer(j),*,iostat=ierr)int_num
...
of z-matrix",i5)')int_num,z
      ...
      if(.not.quiet)write(stdout,'(" Has force constant of type",i5," and
value",f8.3)')i,insprcon(i)
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 subroutine get_cross()
   ...
   integer :: n,i, num_fields,iz, zmat_num,cross_num,int1,int2,z,j,jj
   character(len=10), allocatable :: split_buffer(:)
   ...
   ! this entails reading in CROSSDEF  lines (CROSSDEF ZMAT 1 CROSS 1 INT
2 3 means
   ...
   do i=1,total_num_zm
      if(ncross_given(i).ne.-9999) then
         if(ncross(i).ne.ncross_given(i)) then
            write(stderr,*)'----------------------------------------------------------------------------'
            write(stderr,*)'Number of cross terms specifed using CROSSDEF
not same as number'
   ...
   if (ierr.ne.0) call
allerr('cross_terms(total_num_zm,(maxval(ncross),2)         ')
   ...
   do i = 1,n
      ...
      if((zmat_num.lt.1).or.(zmat_num.gt.total_num_zm))call
fatal_keyword(keyword,buffer)
      if((min(int1,int2).lt.1).or.(max(int1,int2).gt.nintern(zmat_num)))call
fatal_keyword(keyword,buffer)
      if((cross_num.lt.1).or.(cross_num.gt.ncross(zmat_num)))call
fatal_keyword(keyword,buffer)
      text1 = .ucase. text1
      text2 = .ucase. text2
      text3 = .ucase. text3
      ...
      if(.not.quiet)write(stdout,*)'Cross term',cross_num,' of z-matrix
',zmat_num,' is between internal d.f.',int1,' and ',int2
   end do

   !----------------------------------------------------------------------------------
   ! So we have cross_terms defined.  Now we need to fill crosstype and
cross_spr
   ...
   do i = 1,n
      ...
      if(ierr.ne.0) call fatal_keyword(keyword,buffer)
      if((cross_num.lt.1).or.(cross_num.gt.ncross(z)))call
fatal_keyword(keyword,buffer)
      crosstype(cross_num,z) = i
      if (.not.quiet)write(stdout,*)'     Cross term    #',cross_num, 'of
z-matrix',z
      if (num_fields.gt.5) then
         j = 6
         do jj = 6, num_fields
            ...
            if (.isnumber. text) then
               ...
               crosstype(cross_num,z) = i
               if (.not.quiet) write(stdout,'(" and cross term    #",i5,"
of z-matrix",i5)')cross_num,z
               j = j + 1
               if(j.gt.num_fields)exit
            else
               ! It must be next ZMAT statement:
...
fatal_keyword(keyword,buffer)
               ...
               if(ierr.ne.0) call fatal_keyword(keyword,buffer)
               text = .ucase.text
               if(trim(text).ne.trim(cross)) call
fatal_keyword(keyword,buffer)
               ! then int number
               if(j.gt.num_fields)exit
               read(split_buffer(j),*,iostat=ierr)cross_num
...
quiet))write(stdout,*)'----------------------------------------------------------------------------'
 ...
 subroutine longusage()
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| The main input file is a series of keywords and
values.  Some       |'
   write(stdout,*)'| are mandatory, some are not.  The order in the
keyword file does    |'
   write(stdout,*)'| not matter.  The details are outlined below.        
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| It is best to begin all filenames called within the
keyword file    |'
   write(stdout,*)'| with alphaetical characters, not with numbers or
other characters.  |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| ZMC uses a z-matrix to describe a molecule.  If
carefully           |'
   write(stdout,*)'| constructed, this allows segmented motion of the
molecules in a     |'
   write(stdout,*)'| simple way.  The convention for naming these files
is to give       |'
   write(stdout,*)'| them the extension ".zmat".  While a z-matrix
defines the           |'
   write(stdout,*)'| molecular geometry, the molecule must be oriented
and positioned    |'
   write(stdout,*)'| within the unit cell.  This is done using a 3-vector
(x, y, z)      |'
   write(stdout,*)'| to specify the origin of the molecule (the position
of the first    |'
   write(stdout,*)'| atom) and a quaternion (a normalised 4-vector
(q1,q2,q3,q4)) to     |'
   write(stdout,*)'| give the orientation.  This information is in files
with            |'
   write(stdout,*)'| extension ".qxyz". If one uses a third vector to
hold the values    |'
   write(stdout,*)'| of the internal degrees of freedom for the molecule
(this will      |'
   write(stdout,*)'| be an n-vector if there are n internal d.f.) then
the molecule      |'
   write(stdout,*)'| is completely specified by the 3-vector, the
4-vector and the n-    |'
   write(stdout,*)'| vector.  Given that a substantial molecule may have
50 atoms in     |'
   write(stdout,*)'| it, requiring 150 coordinates, this is a great
economy of           |'
   write(stdout,*)'| variables.                                          
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| A molecule is said to occupy a location rather than
a site          |'
   write(stdout,*)'| simply because site is a word commonly attached to
atomic           |'
   write(stdout,*)'| position, and there is a desire to avoid confusion
by using a       |'
   write(stdout,*)'| word (hopefully) without connotations.              
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| A single Monte Carlo (MC) step consists of choosing
a molecule      |'
   write(stdout,*)'| at random, calculating its energy by summing over
any               |'
   write(stdout,*)'| interactions pertaining to it (whether internal to
the molecule     |'
   write(stdout,*)'| or between the molecule and its environment), then
randomly         |'
   write(stdout,*)'| altering the molecules configuration (putting random
shifts on      |'
   write(stdout,*)'| the three vectors noted above), calculating the new
energy and      |'
   write(stdout,*)'| then accepting or rejecting the new configuration
based on a MC     |'
   write(stdout,*)'| algorithm.                                          
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| Hence the keywords allow the user to specify what
the degrees of    |'
   write(stdout,*)'| freedom are, what the force constants (spring
constants) acting     |'
   write(stdout,*)'| in the system are, and how wide the distribution of
random          |'
   write(stdout,*)'| shifts can be (referred to as various kinds of
widths).             |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| It is my intention that a fuller manual for ZMC will
become         |'
   write(stdout,*)'| available with time, and that it will be distributed
with           |'
   write(stdout,*)'| example input files and simulations that work. 
Also, some          |'
   write(stdout,*)'| documentation exists in the reviewed literature, as
noted in        |'
...
--------------------------------------------------------------      |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| CRYSTAL      Specifies the size of the model crystal
in unit        |'
   write(stdout,*)'|              cells. Must be followed by three
integers.             |'
   write(stdout,*)'|              e.g. CRYSTAL 32 16 5                   
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| HEADER       Specifies (if given) a header (up to
100               |'
   write(stdout,*)'|              characters long) for the keyword file
and which        |'
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| TEMPERATURE  Specifies the temperature of the Monte
Carlo           |'
   write(stdout,*)'|              simulation. This affects how likely it
is that a       |'
   write(stdout,*)'|              MC move which increases the system
energy will be      |'
   write(stdout,*)'|              accepted.  Must be followed by one
real.               |'
   write(stdout,*)'|              e.g. TEMPERATURE 1.0                   
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| MCCYCLES     Specifies number of Monte Carlo cycles
to do. One      |'
   write(stdout,*)'|              cycle consists of a number of MC steps
sufficient      |'
   write(stdout,*)'|              to visit every location in the crystal
once.           |'
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| INCUPDATE    Specifies how many MC cycles between
adjusting         |'
   write(stdout,*)'|              the sizes of the increments (widths)
that govern       |'
   write(stdout,*)'|              the shifts put on the variables in the 
              |'
   write(stdout,*)'|              simulation.  The initial randomness is
governed        |'
   write(stdout,*)'|              by XYZINITW, QINITW and ININITW, while
the sizes       |'
   write(stdout,*)'|              of the random shifts applied to the
variables          |'
   write(stdout,*)'|              during the MC are governed by XYZWIDTH,
QWIDTH         |'
   write(stdout,*)'|              and INWIDTH.  These latter parameters
can be           |'
   write(stdout,*)'|              dynamically adjusted to give a
rejection ratio of      |'
   write(stdout,*)'|              about 50% (half moves rejected) which
is a rule        |'
   write(stdout,*)'|              of thumb often applied to MC
simulation.               |'
   write(stdout,*)'|              Further, the subroutine adjusts the
shifts on the      |'
   write(stdout,*)'|              individual variables to ensure that all
are            |'
   write(stdout,*)'|              having approximately equal influence on
the            |'
   write(stdout,*)'|              acceptance/rejection ratio. If it turns
out that       |'
   write(stdout,*)'|              one variable (for example the x
position of the        |'
   write(stdout,*)'|              origin) has a very big width, this
indicates that      |'
   write(stdout,*)'|              the simulation is only weakly sensitive
to shifts      |'
   write(stdout,*)'|              in this direction.  This may reflect
reality, or       |'
   write(stdout,*)'|              may imply more or stronger constraints 
              |'
   write(stdout,*)'|              (interactions) are needed.  Must be
followed by a      |'
   write(stdout,*)'|              single integer.  e.g. INCUPDATE 1      
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| BADJUST      Specifies how many MC cycles between
adjusting         |'
   write(stdout,*)'|              the strengths of the interactions in
the system        |'
   write(stdout,*)'|              to achieve a specified average B-factor
across         |'
   write(stdout,*)'|              ALL atoms.  Interactions are scaled
globally, so       |'
   write(stdout,*)'|              this is analogous to changing the
simulation           |'
   write(stdout,*)'|              temperature.  It is helpful because it
gives a         |'
   write(stdout,*)'|              good indication of the scale of the
interactions.      |'
   write(stdout,*)'|              Since it scales them all at once their
relative        |'
   write(stdout,*)'|              values do not change.  The B-factor
specified          |'
   write(stdout,*)'|              (B = 8pi^2U) is an average across all
atoms in         |'
   write(stdout,*)'|              the z-matrix, so is a very crude
number, and is        |'
   write(stdout,*)'|              calculated as isotropic, which is also
a crude         |'
   write(stdout,*)'|              approximation.  Must be followed by one
integer        |'
   write(stdout,*)'|              and one real, where the real is the
desired            |'
   write(stdout,*)'|              B-factor.  e.g. BADJUST 1 2.5          
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| NUMSPRCON    Specifies the number of different
spring               |'
   write(stdout,*)'|              constants (interaction constants)
acting on            |'
   write(stdout,*)'|              contact vectors in the model.  Will be
deduced         |'
   write(stdout,*)'|              from other inputs if not given.  Must
be followed      |'
   write(stdout,*)'|              by one integer.  e.g. NUMSPRCON 23     
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| NUMINSPRCON  Specifies the number of different
spring               |'
   write(stdout,*)'|              constants (interaction constants)
acting on            |'
   write(stdout,*)'|              internal degrees of freedom in the
model.              |'
   write(stdout,*)'|              Will be deduced from other inputs if
not given.        |'
   write(stdout,*)'|              Must be followed by one integer.       
              |'
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| NUMLOCS      Specifies the number of locations in
the unit          |'
   write(stdout,*)'|              cell.  Location is used instead of site
because        |'
   write(stdout,*)'|              site tends to be used to refer to
atoms; a             |'
   write(stdout,*)'|              location is a position in the unit cell
which is       |'
   write(stdout,*)'|              occupied by a molecule.  The occupation
of a           |'
   write(stdout,*)'|              given location may vary from cell to
cell -- the       |'
   write(stdout,*)'|              type or orientation of z-matrix on the
location        |'
   write(stdout,*)'|              may vary.  Will be deduced from other
inputs if        |'
   write(stdout,*)'|              not given.  Must be  followed by one
integer.          |'
   write(stdout,*)'|              e.g. NUMLOCS 23                        
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| NUMZMATS     Specifies the number of different
z-matrices           |'
   write(stdout,*)'|              present in the simulation.  Will be
deduced from       |'
   write(stdout,*)'|              other inputs if not given.  Must be
followed by        |'
   write(stdout,*)'|              one integer.  e.g. NUMZMATS 2          
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| NUMINTERNAL  Specifies the number of internal
degrees of            |'
   write(stdout,*)'|              freedom a given z-matrix is given. 
Must be            |'
   write(stdout,*)'|              followed by the sub-keyword ZMAT then
two              |'
   write(stdout,*)'|              integers, the ZMAT number (as given in
the             |'
   write(stdout,*)'|              ZMATRIX lines of the keyword file) and
how many        |'
   write(stdout,*)'|              internal degrees of freedom that
z-matrix has          |'
   write(stdout,*)'|              been given.  Will be deduced from other
inputs if      |'
   write(stdout,*)'|              not given.  To specify that z-matrix 2
has 3           |'
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| NUMCROSS     Specifies the number of cross-terms a
given            |'
   write(stdout,*)'|              z-matrix is given. A cross-term is an
interaction      |'
   write(stdout,*)'|              between internal degrees of freedom. 
Must be          |'
   write(stdout,*)'|              followed by the sub-keyword ZMAT then
two              |'
   write(stdout,*)'|              integers, the ZMAT number (as given in
the             |'
   write(stdout,*)'|              ZMATFILE lines of the keyword file) and
how many       |'
   write(stdout,*)'|              cross terms that z-matrix has been
given.  Will        |'
   write(stdout,*)'|              be deduced from other inputs if not
given.  To         |'
   write(stdout,*)'|              specify that z-matrix 2 has 3 internal
cross           |'
   write(stdout,*)'|              terms, write: NUMCROSS ZMAT 2 3        
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| OCCFILE      Specifies the name of the file the
occupancies         |'
   write(stdout,*)'|              are to be read from.  If not given it
is assumed       |'
   write(stdout,*)'|              that it is not needed (a model in which
there is       |'
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| ZMATFILE     Specifies the file containing a
z-matrix.  Must        |'
   write(stdout,*)'|              be followed by an integer, which is the
z-matrix       |'
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| QXYZFILE     Specifies the file containing the
variables            |'
   write(stdout,*)'|              describing the average origin of the
z-matrix          |'
   write(stdout,*)'|              (xyz) and its orientation (quaternion,
q).  Must       |'
   write(stdout,*)'|              be followed by an integer, which is the
z-matrix       |'
   write(stdout,*)'|              number, and a filename.                
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| CELL         Specifies unit cell parameters (angles
in              |'
   write(stdout,*)'|              degrees). Must be followed by 6 reals. 
              |'
   write(stdout,*)'|              e.g.  CELL 12.34 5.126 8.902 90.0 
109.35 90.0         |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| CONTACTFILE  Specifies the file containing the
contact              |'
   write(stdout,*)'|              vectors.  e.g. CONTACTFILE Contacts.txt
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| SPRCON       Specifies value of spring constants
(interaction       |'
   write(stdout,*)'|              constants).  If given with a single
real               |'
   write(stdout,*)'|              (e.g. SPRCON 12.0) sets a default value
that           |'
   write(stdout,*)'|              applies to all springs.  Subsequent
invocations,       |'
   write(stdout,*)'|              with a real followed by integers,      
              |'
   write(stdout,*)'|              (e.g. SPRCON 50.2 1 2 3 4 5 6 7) sets
the spring       |'
   write(stdout,*)'|              constants for those contact vectors
specified.         |'
   write(stdout,*)'|              The two invocations here would set all
springs to      |'
   write(stdout,*)'|              be 12.0, then set the spring constants
on vectors      |'
   write(stdout,*)'|              1 to 7 to be 50.2, leaving the others,
if any, at      |'
   write(stdout,*)'|              12.0.                                  
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| SIZE         Specifies value of size-effects on
contact             |'
   write(stdout,*)'|              vectors.  Invocation rules are the same
as for         |'
   write(stdout,*)'|              SPRCON.                                
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| INSPR        Specifies spring constants on internal
degrees of      |'
   write(stdout,*)'|              freedom.  An example would be:         
              |'
   write(stdout,*)'|              INSPR  50.0  ZMAT 1 INT 1 ZMAT 2 INT 2
3               |'
   write(stdout,*)'|              This would set the spring constant
(interaction        |'
   write(stdout,*)'|              constant) for internal degree of
freedom 1 on          |'
   write(stdout,*)'|              z-matrix 1 to be 50.0 and internal
spring              |'
   write(stdout,*)'|              constants 2 and 3 of z-matrix 2 would
also be          |'
   write(stdout,*)'|              50.0.  These must all be set
explicitly; there is      |'
   write(stdout,*)'|              no defaulting invocation as for SPRCON.
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| INTERNAL     Specifies the nature of an internal
degree of          |'
   write(stdout,*)'|              freedom.  To specify internal degree of
freedom        |'
   write(stdout,*)'|              number 1 for z-matrix number 2 to be
the               |'
   write(stdout,*)'|              dihedral angle of atom 17, type:       
              |'
   write(stdout,*)'|              INTERNAL ZMAT 2 INT 1 dihedral 17      
              |'
   write(stdout,*)'|              The types of internal d.f. are bond
lengths (use       |'
   write(stdout,*)'|              "length" instead of "dihedral"), bond
angle            |'
   write(stdout,*)'|              (sub-keyword "angle") or dihedral angle
as shown.      |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| CROSSSPR     Specifies spring constants on cross
terms.  An         |'
   write(stdout,*)'|              example would be:                      
              |'
   write(stdout,*)'|              CROSSSPR  50.0  ZMAT 1 CROSS 1 ZMAT 2
CROSS 2 3        |'
   ...
   write(stdout,*)'|              CROSSDEF ZMAT 1 CROSS 1 INT 1 2        
              |'
   write(stdout,*)'|              would define the first cross-term of
z-matrix 1        |'
   write(stdout,*)'|              is an interaction between internal d.f.
1 and 2.       |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| XYZWIDTH     Sets the sizes of the random increments
on x, y        |'
   write(stdout,*)'|              and z coordinates of z-matrices.  Can
be invoked       |'
   write(stdout,*)'|              in several ways.  "XYZWIDTH 0.2" would
set widths      |'
   write(stdout,*)'|              to be 0.2 for all x,y and z on all
z-matrices.         |'
   write(stdout,*)'|              "XYZWIDTH X 0.1" would set the widths
on the           |'
   write(stdout,*)'|              x-shifts on all z-matrices to be 0.1.  
              |'
   write(stdout,*)'|              "XYZWIDTH 1 0.15" would set widths on
x, y and z       |'
   write(stdout,*)'|              for all of x, y and z on z-matrix 1 to
be 0.15         |'
   write(stdout,*)'|              and (lastly) "XYZWIDTH 1 Y 0.4" would
set the          |'
   write(stdout,*)'|              width of the y coordinate on z-matrix 1
to            |'
   write(stdout,*)'|              be 0.4.  "XTZWIDTH 1 X Y 0.15" would
not work;         |'
   write(stdout,*)'|              use separate invocations for X and Y in
this case.     |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| XYZINITW     Sets the initial randomness on the
variables;          |'
   write(stdout,*)'|              invocation as for XYZWIDTH.            
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| QWIDTH       Sets the sizes of the random increments
on             |'
   write(stdout,*)'|              quaternion components.  As for XYZWIDTH
but refer      |'
   write(stdout,*)'|              to Q1, Q2, Q3 and Q4 rather than X, Y
and Z.           |'
   write(stdout,*)'|                                                     
              |'
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| INWIDTH      Sets the sizes of the random increments
on             |'
   write(stdout,*)'|              internal degrees of freedom.  As for
XYZWIDTH but      |'
   write(stdout,*)'|              refer to I1, I2, ..., IN rather than X,
Y and Z.       |'
   write(stdout,*)'|              NOTE that it works best with one line
per d.f..        |'
   write(stdout,*)'|              For example "INWIDTH 1 I2 15.0" would
set the width    |'
   write(stdout,*)'|              for internal 2 on ZMAT 1 to be 15.0
degrees.           |'
   write(stdout,*)'|                                                     
              |'
   ...
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| VMIN         When creating some contact vectors,
this is the        |'
   write(stdout,*)'|              minimum length (Angstrom). e.g. VMIN
1.9               |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| VMAX         When creating some contact vectors,
this is the        |'
   write(stdout,*)'|              maximum length.                        
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| NEWCONTACTS  Specifies the new contact vector file
name.            |'
   write(stdout,*)'|              e.g. NEWCONTACTS newfilename.out       
              |'
   write(stdout,*)'|                                                     
              |'
   write(stdout,*)'| CONTATOMS    Specified which atoms to look at when
generating       |'
   write(stdout,*)'|              contact vectors.  "CONTATOMS ZMAT 1 17
25" would       |'
   write(stdout,*)'|              look for contacts between atoms 17 and
25 on           |'
   write(stdout,*)'|              ZMAT 1.  To look for contacts involving
more than      |'
   write(stdout,*)'|              one z-matrix type, use multiple
instances of           |'
 ...
 subroutine write_readat(filename)
   ...
   !----------------------------------------------------------------------------------------------
   ! This suggests that what we need to do is (1) Work out number of
possible sites in a cell.
   ! (2) Work out the maximum number of sites that can be occupied in a
cell.
   ! (3) Write these things and simsize() to the output file.
   ! (4) Loop over the POSSIBLE sites in the cell (by looping over
locations then possible
   !     z-matrix then possible orientations) and increment a counter n
for each non-dummy atom.
   !     Before doing this, set correspond() to zeroes; then, when on a
site which IS occupied
   !     increment a second counter, m, and set correspond(i,j,k,n) = m. 
Then set
   !     x(i,j,k,m), y, z and label() as well, so that they can be
accessed by using
   !     x(i,j,k,correspond(i,j,k,n)) for example.  Then, when doing
readat() it is simply
   !     that if correspond(i,j,k,isite)=0, it is unoccupied.  If .ne.0,
then get label and
   !     compare to type passed in.  If agreement, set the flag, set
x,y,z and get out. Easy!
   !----------------------------------------------------------------------------------------------

   character(len=107)                   :: filename
   integer ::
possible_sites,il,iz,oz,im,iat,occupied_sites,max_occ,num_nondummy,dunit
...
cvec,fvec
   ...
   if (ierr.ne.0) call
allerr('which_mol(num_loc,total_num_zm,maxval(mocc))         ')
   call
get_what_goes_where(how_many_types,which_types,instances_types,which_mol,total_num_zm,num_loc,quiet,key2)
   ...
   do il = 1,num_loc
      do iz = 1,how_many_types(il)
         ...
         do im = 1,instances_types(il,oz)
            ! now, we have to loop over the atoms, because we have to
factor out dummies
            do iat = 1,num_at(oz)
               lable = ''
               lable=labels(z1,iat)
...
cell.                    !
   ...
   ! Should we worry about dummy atoms at this stage?  It will mean our
arrays
   ! come out a little bigger than needed...do we need a subroutine to
work out
   ...
   !----------------------------------------------------------------------------------------------
   ! Now the guts of it.
   ! First write out lookup table array; readat() can interrogate that to
find out
            ...
            do il = 1,num_loc
               ...
               do iat = 1,num_at(z_there)
                  ...
                  if((lable(1:1)).ne.'x') then
                     ...
                     if (in2.lt.1) then
                        write(stdout,*)'All atoms in zmats need to be
numbered!!'
                        write(stdout,*)'E.g. Nb1 or O23'
                        write(stdout,*)'Not halting, but check your
output files!'
                     end if
                     ind = min(2,in2)
            ...
            do il = 1,num_loc
               ...
               do iat = 1,num_at(z_there)
                  ...
                  if((lable(1:1)).ne.'x') then
                     ...
                     in2 = max((scan(lable,'0123456789')-1),1)
                     if (in2.lt.1) then
                        write(stdout,*)in2,'All atoms in zmats need to be
numbered!!'
                        write(stdout,*)'E.g. Nb1 or O23'
                        write(stdout,*)'Not halting, but check your
output files!'
...
',1.0,', ',1.0
   ...
   if (.not. quiet)write(stdout,*)'Writing DISCUS pdf macro to
',trim(macfile)
   dunit = open(macfile)
   write(dunit,'(a63)')'# This comes from a very simple routine in ZMC   
            '
   write(dunit,'(a63)')'# to output a DISCUS-compatible file. Also,
outputs a          '
   write(dunit,'(a63)')'# DISCUS macro file that ought to give a first
pass at an input'
   write(dunit,'(a63)')'# file for pdf and powder calculations. NOT
THOROUGHLY TESTED! '
   write(dunit,'(a1)')'#'
   write(dunit,'(a63)')'# See http://discus.sourceforge.net/             
            '
   write(dunit,'(a1)')'#'
   write(dunit,'(a63)')'# Note the spacegroup is always given as P1 by
ZMC             '
   write(dunit,'(a1)')'#'
   write(dunit,'(a63)')'# Note the Biso is always given as unity and is
then           '
   write(dunit,'(a63)')'# disabled in the macro.                         
            '
   write(dunit,'(a1)')'#'
   write(dunit,'(a63)')'# Note also that DISCUS may not be able to read
in all the     '
   write(dunit,'(a63)')'# atoms in the simulation, so you may have to
reduce the       '
   write(dunit,'(a63)')'# simulation size or recompile DISCUS with bigger
limits.      '
   write(dunit,'(a63)')'# Just truncating the .discus file may not work. 
            '
   write(dunit,'(a1)')'#'
   write(dunit,'(a63)')'# To get a quick look at the outputs, plot in a
spreadsheet   '
   write(dunit,'(a63)')'# or just run gnuplot and type plot
"filename.pdf" (in quotes).'
   write(dunit,'(a1)')'#'
   write(dunit,'(a4)')'read'
...
program ZMC
 ...
 !------------------------------------------------------------------------------!
 !     Keywords for use in input files.  See subroutine longusage() for
details
 !------------------------------------------------------------------------------!
 ...
 if ( .not. have_args() ) then
    write(stderr,*)'----------------------------------------------------------------------------'
    write(stdout,*)'This is ZMC version dated ',trim(cver),', a program
for implementing '
    write(stdout,*)'Monte Carlo simulations of crystal structures,
primarily for the'
    write(stdout,*)'analysis of diffuse scattering.'
    write(stdout,*)' '
    write(stderr,*) "Must provide a keyword file filename or a command
line option."
    write(stdout,*)"Type 'ZMC --help' for some help and 'ZMC --help2' for
more help."
    write(stderr,*) 'Program exiting.'
    write(stderr,*)'----------------------------------------------------------------------------'
    stop
...
primarily for the'
    ...
    write(stdout,*)"D. J. Goossens, A. P. Heerdegen, E. J. Chan & T. R.
Welberry, 'Monte"
    write(stdout,*)"Carlo Modelling of Diffuse Scattering from Single
Crystals: The "
    write(stdout,*)"Program ZMC' Metallurgical and Materials Transactions
A, vol 42A, "
    write(stdout,*)"(2011) 23-31.  DOI:10.1007/s11661-010-0199-1"
    write(stdout,*)' '
    write(stdout,*)'Invoke it with ZMC --help for some help, and ZMC
--help2 for more help.'
    write(stdout,*)'----------------------------------------------------------------------------'
 end if
 keyfile = next_arg()
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 if ( .not. have_args() ) then
    if(.not.quiet) write(stdout,*) "Keyword filename will be used as root
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 if (option_exists('getcontacts'))  then
    ...
    if((i1.lt.1).and.(num_loc_given.eq.-9999)) then
       write(stderr,*)'Must give NUMLOCS and/or QXYZFILE and/or OCCFILE
to specify number of locations.'
       write(stderr,*)'----------------------------------------------------------------------------'
       stop
    end if

    call get_num_loc_qxyz(num_loc_given, num_loc_qxyz,
total_num_zm,key2,quiet)
    ...
    if (ierr.ne.0) call
allerr('translate(num_loc,total_num_zm,maxval_mocc, 3)      ')
    ...
    if (ierr.ne.0) call
allerr('which_mol(num_loc,total_num_zm,maxval_mocc)         ')
    call
get_what_goes_where(how_many_types,which_types,instances_types,which_mol,total_num_zm,num_loc,quiet,key2)
 ...
 if((i1.lt.1).and.(num_loc_given.eq.-9999)) then
    write(stderr,*)'Must give NUMLOCS and/or QXYZFILE and/or OCCFILE to
specify number of locations.'
 ...
 if(i1.lt.1) then
    if(.not.quiet)write(stdout,*)'No occupancy file given.  Will try to
work out'
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
    ...
    call
get_num_loc_occ(num_loc_given,num_loc_qxyz,num_loc_occ,total_num_zm,mol_max,key2,quiet,simsize)
 end if

 if((num_loc_occ.eq.-9999).and.(num_loc_given.eq.-9999).and.(num_loc_qxyz.eq.-9999))
then
    write(stderr,*)'----------------------------------------------------------------------------'
    write(stderr,*)'Number of locations (molecular sites) not given!'
    write(stderr,*)'Program exiting.'
 ...
 if (ierr.ne.0) call
allerr('mocc(simsize(1),simsize(2),simsize(3),num_loc)      ')
 allocate(zocc(simsize(1),simsize(2),simsize(3),num_loc),stat=ierr)
 if (ierr.ne.0) call
allerr('zocc(simsize(1),simsize(2),simsize(3),num_loc)      ')
 allocate(xyzcrystal(simsize(1),simsize(2),simsize(3),num_loc,3),stat=ierr)
 if (ierr.ne.0) call
allerr('xyzcrystal(simsize(1),simsize(2),simsize(3),numloc,3')
 ...
 if (ierr.ne.0) call
allerr('qcrystal(simsize(1),simsize(2),simsize(3),num_loc)  ')
 ...
 if (option_exists('reread')) then
    ...
    if (ierr.ne.0) call
allerr('quatern(num_loc,total_num_zm,maxval(mocc))          ')
    allocate(translate(num_loc,total_num_zm,maxval(mocc),3),stat=ierr)
    if (ierr.ne.0) call
allerr('translate(num_loc,total_num_zm,maxval(mocc),3)      ')
    call read_qxyz(quatern,translate)
    call get_maxval_mocc(maxval_mocc)
    call replicator()
    call replinternal()
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 if (option_exists('plot'))  then
    ...
    if (exists(key2,'CONTACTFILE')) then
       ...
       if (ierr.ne.0) call
allerr('conmol(num_loc,maxval(mocc),total_num_zm)           ')
       con_test = 1
 ...
 if
((.not.option_exists('crystal')).and.(.not.option_exists('diffuse')).and.(.not.option_exists('discus')))
then
    write(stderr,*)'----------------------------------------------------------------------------'
    write(stderr,*) 'Must specify one or more of --crystal, --diffuse,
--discus'
 ...
 if (exists(key2,'CONTACTFILE')) then
    ...
    write(stderr,*)'----------------------------------------------------------------------------'
    write(stderr,*) 'Must provide a list of contact vectors via
CONTACTFILE.'
    write(stderr,*) 'Program exiting.'
    write(stderr,*)'----------------------------------------------------------------------------'
    stop
 ...
 if(.not.quiet) write(stdout,'(" Contact vector file contains",i4,"
contact vector types.")')ntyp
 if (.not.
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 if ((ntyp_given.ne.ntyp).and.(ntyp_given.ne.-9999)) then
    ...
    write(stderr,*)'This is different to number specified by NUMSPRCON:
',ntyp_given
    write(stderr,*)'Program exiting.'
    write(stderr,*)'----------------------------------------------------------------------------'
    stop
 ...
 if(.not.quiet) then
    do i = 1,ntyp
       write(stdout,'(" Contact type: ",i4,"  Sprcon: ",f7.3,"  Size
effect: ",f7.3)')i,sprcon(i),size_effect(i)
    end do
    write(stdout,*)'----------------------------------------------------------------------------'
 end if
 ...
 if((ninspr.ne.ninspr_given).and.(ninspr_given.ne.-9999)) then
    write(stderr,*)'----------------------------------------------------------------------------'
    write(stderr,*)'Number of  internal force constants appears to be:
',ninspr
    write(stderr,*)'This is different to number specified by NUMINSPRCON:
',ninspr_given
    write(stderr,*)'Program exiting.'
    write(stderr,*)'----------------------------------------------------------------------------'
    stop
 ...
 if((ninspr.lt.1).and.(num_value(key2,'INTERNAL').gt.0)) then
    write(stderr,*)'----------------------------------------------------------------------------'
    write(stderr,*)' ',num_value(key2,'INTERNAL'),' internal degrees of
freedom defined.'
 ...
 if(mtyp.ne.ntyp) then
    write(stderr,*)'----------------------------------------------------------------------------'
    write(stderr,*)'Number of contact vector types in contact vector file
does not'
    write(stderr,*)'match the number of types specified in the MC
parameters file.'
 ...
 if (ierr.ne.0) call
allerr('reject(total_num_zm,7+maxval(num_at),div_max)       ')
 reject = 0
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
 ...
 do icy = 1,ncy
    ...
    if(icy.gt.1)then
       if (.not. quiet)write(stdout,'(1x," acc:",i8,", up:",i8,",
rej:",i8,", R/T:",f5.2)')  &
            sum(iacc),sum(iup),sum(irej),real(sum(irej)) /
real(sum(iacc)+sum(iup)+sum(irej))
    ...
    do ic2 = 1, mcell
       ...
       qcrystal(oa,ob,oc,ol)        = qcrystal(oa,ob,oc,ol)        +
tweak(4:7)
       xyzcrystal(oa,ob,oc,ol,:)    = xyzcrystal(oa,ob,oc,ol,:)    +
tweak(1:3)
       incrystal(oa,ob,oc,ol,1:nin) = incrystal(oa,ob,oc,ol,1:nin) +
tweak(7+1:7+nin)
      ...
      &
       ...
       do iat = 1, num_at(oz)
          carts(:,oa,ob,oc,ol,iat)=
new_coords(:,iat)+xyzcrystal(oa,ob,oc,ol,:)
       end do
       ...
       if(edif.lt.0.0) then
          ...
          if(rvar(1).le.exp(-edif/temp)) then
             ...
             do iivar = 1,7
                ibin  =
min(int(abs(tweak(iivar))*2.0*rdiv_max/widths(iivar,oz))+1,div_max)
                ibin = max(ibin,1)
                reject(oz,iivar,ibin) = reject(oz,iivar,ibin)+1
             end do
             do iivar = 1,nin
                ibin  =
min(int(abs(tweak(iivar+7))*2.0*rdiv_max/inwidths(iivar,oz))+1,div_max)
                ibin = max(ibin,1)
    ...
    if (.not. quiet)write(stdout,'(1x," %done:",f5.1)') 
real(icy)/real(ncy)*100.0
 end do
    ...
    if (adjcy.gt.0) then
       do iz = 1,total_num_zm
          if (.not. quiet) then
             ...
             write(stdout,'(1x,"Widths for increments on x,y,z are      
",3f9.5)') &
                  (widths(i,iz),i=1,3)
             write(stdout,'(1x,"Widths for increments on q1,q2,q3,q4 are
",4f9.5)') &
                  (widths(i,iz),i=4,7)
          end if
          if(nintern(iz).gt.0) then
             do i = 1,nintern(iz)
                if (.not. quiet) write(stdout,*)'Widths for internal
degrees of freedom:'
                if(intern(i,1,iz).eq.1) then
                 ...
                 ',inwidths(i,iz)
                   ...
                   if (.not. quiet) write(stdout,*)'                     
                 ',inwidths(i,iz)*180.0/3.14159
                end if
             end do
             write(stdout,*)'For internal d.f. that are angles, widths
are expressed in degrees.'
    ...
    if (badjcy.gt.0) then
       if (.not. quiet)
write(stdout,*)'----------------------------------------------------------------------------'
       if (.not. quiet)  write(stdout,*)'Globally scaled intermolecular
spring constants are:'
       ...
       if(ninspr.gt.0) then
          if (.not. quiet)
write(stdout,*)'----------------------------------------------------------------------------'
          if (.not. quiet) write(stdout,*)'Globally scaled intramolecular
spring constants are:'
          do iz = 1,ninspr
       ...
       if(maxval(ncross).gt.0) then
          if (.not. quiet)
write(stdout,*)'----------------------------------------------------------------------------'
          if (.not. quiet) write(stdout,*)'Globally scaled cross term
spring constants are:'
...
quiet)write(stdout,*)'----------------------------------------------------------------------------'
    ...
    if (option_exists('crystal'))  then
       ! get the name from the value of crystal; if it does not have a
value, derive it from outname.
       ...
       if (.not. quiet)write(stdout,*)'Writing ZMC crystal file to
',trim(fname2)
    ...
    if (option_exists('diffuse'))  then
       ...
       if (.not. quiet)write(stdout,*)'Writing file for DIFFUSE
calculation ',trim(fname2)
       call write_readat(fname2)
    end if
    ! And for discus (not implemented yet)`
 ...
 !------------------------------------------------------------------------------!
 ! When the MC has ended, we need to dip into a routine to do some
analysis of it.
 ...
 if (option_exists('energy')) then
    ...
    if (.not. quiet)write(stdout,*)'Writing out molecular energies to
',fname2
    dunit = open(fname2)
    do oa=1,simsize(1)
       do ob=1,simsize(2)
