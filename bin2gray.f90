program bin2gray
  ...
  character(len=*), parameter :: rversion = "$Id: bin2gray.f90,v 1.6 2007/06/04 05:23:11 aidan Exp $"
...
!  integer, external :: nb
  ...
  do
     ...
     if (verbose) then
        ...
        write(stdout,'(A,3(I0,2X))') '   Crystal size is: ',(csize(i), i=1,3)
        write(stdout,'(A,3(I0,2X))') '   Lot Size is: ',(ls_xyz(i), i=1,3)
...
contains
  ...
  subroutine print_version(unit)
    ...
    write(myunit,*) rversion(6:index(rversion,":",back=.true.)-1)
