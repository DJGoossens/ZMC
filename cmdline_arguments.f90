module cmdline_arguments
  ...
  ! Some mods in 2026 to avoid seg faults. grep for DJG
...
contains
  ...
  subroutine process_options (options, status)
    ...
    ! DJG 2026; break this if into 2 parts, so it only evaluates the
    ! second if the first is true; when I run the code, I get an error here. Some
    ! compilers don't evaluate part 2 of an 'and' if part 1 is false, but
    ! maybe this one does? Dunno, but it works now. Makes some sense. If the
    ! thing is not associated, the second part of the AND is undefined.

    if (associated(this_argument_object%bad_option_list)) then
            if (size(this_argument_object%bad_option_list) > 0) then
