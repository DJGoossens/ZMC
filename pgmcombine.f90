program pgmcombine
  ...
  integer, dimension(:,:), allocatable :: data, datatmp, icount
  ...
  allocate(data(maxx,maxy),datatmp(maxx, maxy),icount(maxx,maxy))
  ...
  where (icount /= 0) data = nint(real(data)/real(icount))
