module string_functions
  ...
  ! Some modifications in 2026 to get it to compile and avoid seg faults due to
  ! loose array handling that gfortran no longer allows. Not fully tested.
  ! darren.goossens@gmail.com  grep for DJG to see some edits
...
!! DJG allocate instead of size in defn fixes a seg fault.
!!    type(varying_string), dimension(count(string,delimiter)+1) :: split_string
    type(varying_string), allocatable :: split_string(:)
    ...
    integer :: split_index, i, ierr

!! DJG
    allocate(split_string(count(string,delimiter)+1),stat=ierr)
    if (ierr.ne.0) stop 'allocate split_string'
